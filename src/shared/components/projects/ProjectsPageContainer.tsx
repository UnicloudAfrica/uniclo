...
import ProjectsFilterPanel, { ProjectFilters } from "./ProjectsFilterPanel";
 ...
 */
const ProjectsPageContainer: React.FC<ProjectsPageContainerProps> = ({
  // Data props
  projects = [],
  isLoading = false,
  isFetching = false,
  ...
  useEffect(() => {
    if (!isLoading && projects.length > 0 && !lastUpdatedAt) {
  ...
  const handleSelectProject = useCallback((ids: string[]) => {
  ...
  const handleSelectAll = useCallback(() => {
    if (selectedProjects.length === paginatedProjects.length) {
      ...
      setSelectedProjects(paginatedProjects.map((p: Project) => p.identifier));
    }
    ...
    <div className="space-y-6">
      ...
      {showBulkActions && selectedProjects.length > 0 && (
        <ProjectsBulkActions
          ...
          onArchive={onBulkArchive ? () => onBulkArchive(selectedProjects) : undefined}
          onActivate={onBulkActivate ? () => onBulkActivate(selectedProjects) : undefined}
          onDelete={onBulkDelete ? () => onBulkDelete(selectedProjects) : undefined}
          onExport={enableExport && onBulkExport ? () => onBulkExport(selectedProjects) : undefined}
        />
      )}
