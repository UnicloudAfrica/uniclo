...
const ACTION_LIBRARY = {
  ...
  stop: {
    ...
    icon: Square,
    tone: "warning",
    disableOnStatus: (status) => !["running", "active"].includes((status || "").toLowerCase()),
  },
  reboot: {
    label: "Reboot",
...
const formatMoney = (amount: any, currencyCode: any) => {
...
const formatStatusText = (value: any) => {
  if (!value) return "N/A";
  return value
    .toString()
    .replace(/_/g, " ")
...
const AdminInstancesDetails = () => {
  ...
  const { mutateAsync: refreshStatusMutation, isPending: isRefreshingStatus } =
    useRefreshInstanceStatus();
  const { data: usageStats, isFetching: isUsageLoading } = useInstanceUsageStats(
    instanceIdentifier,
    usagePeriod,
    {
  ...
  const pricingBreakdownRaw = useMemo(() => {
    ...
    return (
      safeParseJson(effectiveMetadata.pricing_breakdown, effectiveMetadata.pricing_breakdown) ||
  ...
  const networkTopologySummary = useMemo(() => {
    if (networkInfo && typeof networkInfo === "object") {
      ...
      if (Array.isArray(networkInfo.networks)) {
        networkInfo.networks.forEach((network: any) => {
      ...
      return {
        ...
        flatAddresses: networkInfo.flat_addresses || networkInfo.flatAddresses || [],
  ...
  const lifecycleEvents = useMemo(() => {
    ...
    rawEvents.forEach((event, index) => {
      ...
      const timestamp =
        event.timestamp || event.occurred_at || event.created_at || event.updated_at || event.date;
      const description =
        event.description || event.details || event.message || event.note || event.summary;
  ...
  const telemetrySummary = useMemo(() => {
    ...
    const cpuMetric =
      telemetry?.cpu_usage ?? telemetry?.cpu ?? metricsSource.cpu_usage ?? metricsSource.cpu;
  ...
  const currency = pricingBreakdownRaw?.currency || displayInstance?.currency || "NGN";
  ...
  const parsedPricingBreakdown = useMemo(() => {
    ...
    const normalizedLines = linesArray.map((line, index) => ({
      ...
      unitAmount: line?.unit_amount ?? line?.unitAmount ?? line?.unit_price ?? line?.price ?? 0,
    ...
    return {
      ...
      subtotal:
        pricing?.subtotal ?? pricing?.pre_discount_subtotal ?? pricing?.preDiscountSubtotal ?? 0,
      discount: pricing?.discount ?? 0,
      discountLabel: pricing?.discount_label ?? pricing?.discountLabel,
      tax: pricing?.tax ?? 0,
      total: pricing?.total ?? 0,
      colocationPercentage: pricing?.colocation_percentage ?? pricing?.facility_percentage ?? null,
      colocationAmount: pricing?.colocation_amount ?? pricing?.facility_amount ?? null,
    };
  ...
  const subscriptionInsights = useMemo(() => {
    ...
    return [
      ...
      },
      {
        label: "Grace Days",
        value:
          displayInstance.grace_days !== undefined && displayInstance.grace_days !== null
  ...
  const relatedResources = useMemo(() => {
    ...
    if (effectiveMetadata?.data_volumes?.length) {
      const volumes = effectiveMetadata.data_volumes.slice(0, 3).map((vol: any) => ({
        id: vol?.id || vol?.name || vol?.volume_label,
        name: vol?.name || vol?.volume_label || "Volume",
        size: vol?.size_gb || vol?.volume_size_gb || vol?.storage_size_gb || vol?.capacity_gb,
      }));
  ...
  const enhancedTransactions = useMemo(() => {
    ...
    return displayInstance.transactions.map((tx: any) => {
      ...
      const breakdownLines = Array.isArray(breakdown?.lines) ? breakdown.lines : [];
  ...
  const handleRefreshStatus = useCallback(async () => {
    ...
    try {
      ...
      ToastUtils.error(error?.message || "Failed to refresh instance status.");
    } finally {
  ...
  const handleInstanceAction = useCallback(
    async (actionKey) => {
      ...
      try {
        ...
        ToastUtils.error(error?.message || `Unable to trigger ${actionKey} action right now.`);
      } finally {
        setPendingAction(null);
      }
    },
  ...
  const handleOpenConsole = useCallback(async () => {
    ...
    try {
      ...
      ToastUtils.error(error?.message || "Unable to open console for this instance.");
    } finally {
  ...
  const handleMetadataSubmit = useCallback(
    async (event) => {
      ...
      try {
        ...
        ToastUtils.error(error?.message || "Unable to update metadata right now.");
      }
    },
    [
      instanceIdentifier,
    ...
    return [
      {
        ...
        value:
          ...
          formatStatusText(displayInstance.status),
      ...
      },
      {
        ...
        title: "Compute Class",
        value:
          displayInstance?.compute?.productable_name || displayInstance?.compute?.name || "N/A",
        icon: <Server size={24} />,
        color: "info",
        description: cores
          ? `${cores} vCPU • ${memoryMb ? `${Math.round(Number(memoryMb) / 1024)} GiB RAM` : "—"}`
          : "Compute profile",
      },
      {
        key: "storage",
        ...
        color: "info",
        description:
          effectiveMetadata?.storage_type || displayInstance?.volume_type?.name || "Block storage",
      },
      {
        key: "cost",
              ...
              <div className="flex flex-wrap items-center gap-2">
                {displayInstance.status && (
                  <StatusPill
                    ...
                    tone={
                      ["running", "active"].includes((displayInstance.status || "").toLowerCase())
                        ? "success"
        ...
        <ModernCard padding="xl" className="space-y-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {USAGE_PERIOD_OPTIONS.map((option: any) => (
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {telemetrySummary.map((metric: any) => (
              <div
                key={metric.label}
                className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3"
              >
          ...
          {isUsageLoading ? (
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {[
                {
                  label: "CPU Average",
                  value: usageStats.cpu_average ? formatPercentage(usageStats.cpu_average) : "N/A",
                },
                {
                  label: "Memory Average",
                  value: usageStats.memory_average ? `${usageStats.memory_average} MB` : "N/A",
                },
                {
                  label: "Network In",
                  value: usageStats.network_in ? `${usageStats.network_in} MB` : "N/A",
                },
                {
                  label: "Network Out",
                  value: usageStats.network_out ? `${usageStats.network_out} MB` : "N/A",
                },
                {
                  label: "Disk Read",
                  value: usageStats.disk_read ? `${usageStats.disk_read} MB` : "N/A",
                },
                {
                  label: "Disk Write",
                  value: usageStats.disk_write ? `${usageStats.disk_write} MB` : "N/A",
                },
              ].map((item: any) => (
                <div
                  key={item.label}
          ...
          <ModernCard padding="xl" className="space-y-6">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <select
                  ...
                  className="rounded-full border border-slate-200 bg-white px-3 py-1 text-sm text-slate-700 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
                >
                  {LOG_LINE_OPTIONS.map((option: any) => (
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-900/95 p-4 text-xs font-mono text-slate-100">
              {isLogsLoading ? (
                ...
                <span className="text-slate-400">No log lines returned for this interval.</span>
              )}
            </div>
          </ModernCard>
        </div>
          ...
          {parsedPricingBreakdown ? (
            <div className="space-y-6">
              ...
              {parsedPricingBreakdown.lines.length ? (
                <ModernTable
                  data={parsedPricingBreakdown.lines.map((line) => ({ ...line, id: line.key }))}
                  columns={[
                    {
                      key: "name",
                      header: "LINE ITEM",
                    ...
                    },
                    {
                      ...
                      header: "UNIT AMOUNT",
                      render: (val, line) => (
                        <span className="text-slate-700">{formatMoney(val, line.currency)}</span>
                      ),
                    },
                    {
                      key: "total",
                      header: "TOTAL",
                      render: (val, line) => (
                        <span className="text-slate-900">{formatMoney(val, line.currency)}</span>
                      ),
                    },
                  ]}
                  searchable={false}
        ...
        <ModernCard padding="xl" className="space-y-6">
          ...
          <div className="overflow-x-auto">
            {enhancedTransactions.length ? (
              <ModernTable
                data={enhancedTransactions.map((tx) => ({ ...tx, id: tx.id || tx.reference }))}
                columns={[
                  {
                    key: "identifier",
                    header: "IDENTIFIER",
                    render: (_, tx) => (
                      <span className="font-medium">{tx.identifier || tx.reference || "—"}</span>
                    ),
                  },
                  {
                    key: "type",
                  ...
                  },
                  {
                    key: "amount",
                    header: "AMOUNT",
                    render: (val, tx) => <span>{formatMoney(val, tx.currency || currency)}</span>,
                  },
                  {
                    key: "status",
                    header: "STATUS",
                    render: (val, tx) => (
                      <StatusPill
                        label={formatStatusText(val)}
                        tone={
                          val && ["success", "paid", "completed"].includes(val.toLowerCase())
                            ? "success"
                            : val && ["failed", "refunded"].includes(val.toLowerCase())
                              ? "danger"
                              : "neutral"
                        }
                      />
                  ...
                  },
                  {
                    key: "payment_gateway",
                    header: "GATEWAY",
                    render: (_, tx) => <span>{tx.payment_gateway || tx.gateway || "—"}</span>,
                  },
                  {
                    key: "created_at",
                    header: "CREATED",
                    render: (val) => <span>{val ? formatDateTime(val) : "—"}</span>,
                  },
                ]}
                searchable={false}
                filterable={false}
