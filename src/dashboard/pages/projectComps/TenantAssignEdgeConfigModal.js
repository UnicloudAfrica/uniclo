...
const TenantAssignEdgeConfigModal = ({ isOpen, onClose, onSuccess, projectId, region }) => {
    ...
    const { data: regions, isFetching: isFetchingRegions } = useFetchTenantRegions();

    const { data: currentConfig } = useFetchTenantProjectEdgeConfig(projectId, selectedRegion, { enabled: isOpen && !!selectedRegion });
    const metadata = currentConfig?.metadata || {};
    const { data: edgeNetworks, isFetching: isFetchingNetworks, error: networksError, refetch: refetchNetworks } = useFetchTenantEdgeNetworks(projectId, selectedRegion, { enabled: isOpen && !!selectedRegion });
    const { data: ipPools, isFetching: isFetchingPools, error: poolsError, refetch: refetchPools } = useFetchTenantIpPools(projectId, selectedRegion, formData.edge_network_id, { enabled: isOpen && !!selectedRegion && !!formData.edge_network_id });
...
export default TenantAssignEdgeConfigModal;
