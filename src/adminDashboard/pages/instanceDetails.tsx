...
const StatusBadge = ({ status, providerStatus, taskState }: any) => {
...
const MetricCard = ({ title, value, unit, trend, icon: Icon, color = "blue" }: any) => {
  const colorClasses = {
...
export default function InstanceDetails() {
  ...
  const fetchInstanceDetails = useCallback(async (identifier) => {
    ...
    try {
      ...
      try {
        ...
        const fallbackPath = `/instances?identifier=${encodeURIComponent(identifier)}`;
  ...
  const executeAction = async (action, params = {}) => {
    ...
    try {
      ...
      ToastUtils.warning(
        `${action} action is no longer available - instance actions have been moved to standard instance management`
      );
      ...
      if (action === "destroy") {
        ...
        if (confirmed) {
          const response = await fetch(
            `${config.adminURL}/instances/${encodeURIComponent(targetIdentifier)}`,
            {
              method: "DELETE",
              headers: {
        ...
        ToastUtils.info(
          `${action} functionality will be available in the updated instance management interface`
        );
      }
    } catch (err) {
  ...
  const instanceDisplayName = instance?.name || `Instance ${instanceReference || ""}`.trim();
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <MetricCard
                  title="Uptime"
                  value={
                    provider_details?.created
                      ? Math.ceil(
                          (new Date() - new Date(provider_details.created)) / (1000 * 60 * 60 * 24)
                        )
                      : "N/A"
                  }
                ...
                <div className="bg-white rounded-lg p-6 border border-gray-200">
                  ...
                  <dl className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <dd className="text-sm text-gray-900 col-span-2">
                        {instance.compute?.memory_mb
                          ? `${Math.round(instance.compute.memory_mb / 1024)} GB`
                          : "N/A"}
                      </dd>
                    </div>
                    <div className="grid grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-lg p-6 border border-gray-200">
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-gray-400 mt-2">
                  Real-time metrics integration coming soon
                </p>
              </div>
            </div>
          )}
              ...
              <div className="bg-gray-900 text-green-400 p-4 rounded-lg font-mono text-sm h-96 overflow-y-auto">
                <div className="space-y-1">
                  ...
                  <div className="text-gray-500">
                    • Log streaming will be implemented with real API integration •
                  </div>
                </div>
              </div>
            </div>
