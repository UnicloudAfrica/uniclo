...
const EditProjectModal = ({
  ...
  const [formData, setFormData] = useState({
    ...
    client_ids: projectDetails?.clients?.map((client: any) => client.id) || [], // Initialize with client IDs
  });
  ...
  useEffect(() => {
    if (projectDetails) {
      setFormData({
        ...
        type: projectDetails.type || "vpc",
        tenant_id: projectDetails.tenant_id || "",
        client_ids: projectDetails.clients?.map((client: any) => client.id) || [],
      });
    ...
    updateProject(
      ...
      },
      {
        ...
        onError: (err) => {
          ...
          ToastUtils.error(err?.message || "Failed to update project. Please try again.");
          setErrors((prev) => ({
            ...prev,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1100] font-Outfit">
      <div className="bg-white rounded-[24px] max-w-[650px] mx-4 w-full   overflow-y-auto">
        ...
        <div className="space-y-4  max-h-[400px]  px-6 py-4 w-full overflow-y-auto">
          ...
          <div>
            <label htmlFor="client_ids" className="block text-sm font-medium text-gray-700 mb-2">
              Client(s) (Optional)
            </label>
            <DropdownSelect
              options={clients || []}
              ...
              disabled={isPending} // Disable if saving
            />
            {errors.client_ids && <p className="text-red-500 text-xs mt-1">{errors.client_ids}</p>}
          </div>
