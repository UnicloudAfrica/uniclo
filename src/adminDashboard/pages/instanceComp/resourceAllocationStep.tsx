  ...
  return (
    <div className="w-full">
      <div className="text-center mb-8">
        <h3 className="text-xl font-semibold text-gray-800">Resource Allocation</h3>
        <p className="text-sm text-gray-500 mt-1 max-w-2xl mx-auto">
          Configure the resources for one or more instances. You can add multiple configurations to
          this request before proceeding.
        </p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 lg:gap-12">
        {/* Left Column: Form */}
        <div className="space-y-4">
          <h4 className="text-lg font-semibold text-gray-700 border-b pb-2">Add a Configuration</h4>
          <div>
            ...
            >
              {projects && projects.length > 0 ? (
                <select
                  ...
                  onChange={(e) => handleSelectChange("selectedProject", e.target.value, projects)}
          ...
          >
            <option value="">{formData.selectedProject?.region || "Select a project"}</option>
          </select>
          ...
          <div>
            ...
            <span
              className={`w-full input-field block transition-all ${
                errors.selectedComputeInstance ? "border-red-500 border" : "border-gray-300"
              }`}
            >
              {isComputerInstancesFetching ? (
                <div className="flex items-center ">
                  <Loader2 className="w-4 h-4 animate-spin mr-2 text-gray-500" />
                  <span className="text-gray-500 text-sm">Loading compute instances...</span>
                </div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleSelectChange("selectedComputeInstance", e.target.value, computerInstances)
                  }
                  className="w-full bg-transparent outline-none "
                  disabled={isSubmissionPending}
                >
            ...
            {errors.selectedComputeInstance && (
              <p className="text-red-500 text-xs mt-1">{errors.selectedComputeInstance}</p>
            )}
          </div>
          <div>
            <label htmlFor="ebs_volume_id" className="block text-sm font-medium text-gray-700 mb-2">
              EBS Volume<span className="text-red-500">*</span>
            </label>
            <span
              className={`w-full input-field block transition-all ${
                errors.selectedEbsVolume ? "border-red-500 border" : "border-gray-300"
              }`}
            >
              {isEbsVolumesFetching ? (
                <div className="flex items-center ">
                  <Loader2 className="w-4 h-4 animate-spin mr-2 text-gray-500" />
                  <span className="text-gray-500 text-sm">Loading EBS volumes...</span>
                </div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleSelectChange("selectedEbsVolume", e.target.value, ebsVolumes)
                  }
                  className="w-full bg-transparent outline-none "
                  disabled={isSubmissionPending}
                >
            ...
            {errors.selectedEbsVolume && (
              <p className="text-red-500 text-xs mt-1">{errors.selectedEbsVolume}</p>
            )}
          </div>
          <div>
            <label htmlFor="os_image_id" className="block text-sm font-medium text-gray-700 mb-2">
            ...
            >
              {isOsImagesFetching ? (
                <div className="flex items-center ">
                  <Loader2 className="w-4 h-4 animate-spin mr-2 text-gray-500" />
                  <span className="text-gray-500 text-sm">Loading OS images...</span>
                </div>
              ) : osImages && osImages.length > 0 ? (
                <select
                  id="os_image_id"
                  value={formData.selectedOsImage?.product?.id || ""}
                  onChange={(e) => handleSelectChange("selectedOsImage", e.target.value, osImages)}
                  className="w-full bg-transparent outline-none "
                  disabled={isSubmissionPending}
                >
          ...
          <div>
            <label htmlFor="keypair_name" className="block text-sm font-medium text-gray-700 mb-2">
            ...
            >
              {isKeyPairsFetching ? (
                <div className="flex items-center ">
                  <Loader2 className="w-4 h-4 animate-spin mr-2 text-gray-500" />
                  <span className="text-gray-500 text-sm">Loading key pairs...</span>
                </div>
              ) : keyPairs && keyPairs.length > 0 ? (
                <select
                  id="keypair_name"
                  value={formData.keypair_name || ""}
                  onChange={(e) => handleSelectChange("keypair_name", e.target.value)}
                  className="w-full bg-transparent outline-none "
                  disabled={isSubmissionPending}
                >
          ...
          {/* Dummy Network ID */}
          <div>
            <label htmlFor="network_id" className="block text-sm font-medium text-gray-700 mb-2">
              Network
            </label>
            <select
              id="network_id"
          ...
          {/* Subnet ID */}
          <div>
            <label htmlFor="subnet_id" className="block text-sm font-medium text-gray-700 mb-2">
              Subnet
            </label>
            <span
              className={`w-full input-field block transition-all ${
            ...
            >
              {isSubnetsFetching ? (
                <div className="flex items-center ">
                  <Loader2 className="w-4 h-4 animate-spin mr-2 text-gray-500" />
                  <span className="text-gray-500 text-sm">Loading subnets...</span>
                </div>
              ) : subnets && subnets.length > 0 ? (
                <select
                  id="subnet_id"
                  value={formData.subnet_id || ""}
                  onChange={(e) => handleSelectChange("subnet_id", e.target.value)}
                  className="w-full bg-transparent outline-none "
                  disabled={isSubmissionPending}
                >
            ...
            {errors.subnet_id && <p className="text-red-500 text-xs mt-1">{errors.subnet_id}</p>}
          </div>
          ...
          <div>
            {isSecurityGroupsFetching ? (
              <div className="flex items-center ">
                <Loader2 className="w-4 h-4 animate-spin mr-2 text-gray-500" />
                <span className="text-gray-500 text-sm">Loading security groups...</span>
              </div>
            ) : securityGroups && securityGroups.length > 0 ? (
              <CheckGroup
                label="Security Groups"
                options={securityGroups.map((sg: any) => ({
                  label: sg.name,
                  value: sg.id,
                }))}
                selectedValues={formData.security_group_ids}
                onChange={(sgId) => handleCheckboxChange("security_group_ids", sgId)}
            ...
            <div>
              ...
              >
                {isBandwidthsFetching ? (
                  <div className="flex items-center ">
                    <Loader2 className="w-4 h-4 animate-spin mr-2 text-gray-500" />
                    <span className="text-gray-500 text-sm">Loading bandwidth...</span>
                  </div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      updateFormData("bandwidth_id", selectedValue);
                      updateFormData("bandwidth_count", selectedValue ? 1 : 0);
                    }}
                    className="w-full bg-transparent outline-none "
                    disabled={isSubmissionPending}
                  >
            ...
            {formData.bandwidth_id && (
              <div>
                ...
                {errors.bandwidth_count && (
                  <p className="text-red-500 text-xs mt-1">{errors.bandwidth_count}</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <span
                className={`w-full input-field block transition-all ${
                  errors.floating_ip_id ? "border-red-500 border" : "border-gray-300"
                }`}
              >
                {isFloatingIpsFetching ? (
                  <div className="flex items-center ">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const selectedValue = e.target.value;
                      updateFormData("floating_ip_id", selectedValue);
                      updateFormData("floating_ip_count", selectedValue ? 1 : 0);
                    }}
                    className="w-full bg-transparent outline-none "
                    disabled={isSubmissionPending}
                  >
            ...
            {formData.floating_ip_id && (
              <div>
                ...
                {errors.floating_ip_count && (
                  <p className="text-red-500 text-xs mt-1">{errors.floating_ip_count}</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <span
                className={`w-full input-field block transition-all ${
                  errors.cross_connect_id ? "border-red-500 border" : "border-gray-300"
                }`}
              >
                {isCrossConnectsFetching ? (
                  <div className="flex items-center ">
                    <Loader2 className="w-4 h-4 animate-spin mr-2 text-gray-500" />
                    <span className="text-gray-500 text-sm">Loading cross connects...</span>
                  </div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const selectedValue = e.target.value;
                      updateFormData("cross_connect_id", selectedValue);
                      updateFormData("cross_connect_count", selectedValue ? 1 : 0);
                    }}
                    className="w-full bg-transparent outline-none "
                    disabled={isSubmissionPending}
                  >
            ...
            {formData.cross_connect_id && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateFormData("cross_connect_count", e.target.value)}
                  placeholder="Enter count"
                  min="1"
                  className={`w-full input-field ${
                    errors.cross_connect_count ? "border-red-500" : "border-gray-300"
                  }`}
                />
                {errors.cross_connect_count && (
                  <p className="text-red-500 text-xs mt-1">{errors.cross_connect_count}</p>
                )}
              </div>
            )}
          </div>
        ...
        <div className="mt-8 lg:mt-0">
          <div className="sticky top-28">
            <div className="bg-gray-50 rounded-lg p-4 border">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">Instance Configurations</h3>
                  ...
                  {pricingRequests.map((req, index) => (
                    ...
                    >
                      <span className="font-medium text-gray-700">
                        {req.name} ({req.number_of_instances}x {req._display.compute},{" "}
