...
import { TrendingUp, Package, MapPin, Tag, Receipt, type LucideIcon } from "lucide-react";
import { PricingRequest, ObjectStorageRequest, Totals } from "../types";
...
type StatColor = "primary" | "blue" | "amber" | "emerald" | "green";
...
const InvoiceStatsCard: React.FC<InvoiceStatsCardProps> = ({
  items = [],
  discount = null,
  totals = null,
}) => {
  const computeItems = items.filter(
    (item): item is PricingRequest => "compute_instance_id" in item
  );
  const storageItems = items.filter(
    (item): item is ObjectStorageRequest => "productable_id" in item
  );
  ...
  const uniqueRegions = Array.from(new Set(items.map((item) => item.region).filter(Boolean)));
