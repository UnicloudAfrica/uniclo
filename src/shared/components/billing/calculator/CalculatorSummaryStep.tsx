...
const CalculatorSummaryStep: React.FC<CalculatorSummaryStepProps> = ({
  calculatorData,
  pricingResult,
  onRecalculate,
}) => {
  ...
  const normalizeLine = (line: any) => ({
    ...
    unit_price: Number(line.unit_price ?? line.unit_amount ?? 0),
  ...
  const apiStorageLines = hasApiStorage ? apiStorage.lines.map(normalizeLine) : [];
  ...
  const combinedMonthly = Number(pricingMeta?.monthly_total || 0) + storageMonthlyWithTax;
  ...
  const storageConfigurationCount = hasApiStorage ? storageLines.length : storageItems.length;
  ...
  const invoicePayloadBase = (additional: Record<string, any> = {}) => {
    ...
    const payload: any = {
      ...
      country_code: calculatorData.country_code,
      ...
      total_discount: calculatorData.total_discount_value
        ? Number(calculatorData.total_discount_value)
        : 0,
      total_discount_label: total_discount_label,
    };
    ...
    payload.total_discount = {
      ...
      value: Number.parseFloat(String(calculatorData.total_discount_value)),
      label: total_discount_label || null,
    };
  ...
  const generateInvoice = () => {
    const errors_msg: Record<string, string> = {};
    if (!invoiceData["subject"]) errors_msg["subject"] = "Subject is required";
    if (!invoiceData["email"]) errors_msg["email"] = "Recipient email is required";
    if (!invoiceData["bill_to_name"]) errors_msg["bill_to_name"] = "Bill-to contact is required";

    if (Object.keys(errors_msg).length) {
      setFormErrors((prev) => ({ ...prev, invoice: errors_msg }));
      return;
    }
  ...
  const createLead = () => {
    const errors_msg: Record<string, string> = {};
    if (!leadData["first_name"]) errors_msg["first_name"] = "First name required";
    if (!leadData["last_name"]) errors_msg["last_name"] = "Last name required";
    if (!leadData["email"]) errors_msg["email"] = "Email required";

    if (Object.keys(errors_msg).length) {
      setFormErrors((prev) => ({ ...prev, lead: errors_msg }));
      return;
    }
  ...
  const statTiles = [
    ...
    },
    {
      ...
      value: (() => {
        ...
        return calculatorData.total_discount_type === "percent"
          ? `${calculatorData.total_discount_value}%`
          : formatCurrency(Number(calculatorData.total_discount_value), primaryCurrency);
      })(),
      ...
      <ModernCard padding="lg" className="space-y-6">
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold text-slate-900">Pricing result</h3>
              <p className="text-sm text-slate-500">
                Review the calculated totals, adjust anything that looks off, and optionally create
                follow-up actions.
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <p className="mt-1 text-lg font-semibold text-slate-900">
              {combinedMonthly > 0 ? formatCurrency(combinedMonthly, primaryCurrency) : "—"}
            </p>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <p className="mt-1 text-lg font-semibold text-slate-900">{primaryCurrency}</p>
        ...
        <ModernCard padding="lg" className="space-y-5">
          ...
          {showInvoiceOptions ? (
            <div className="space-y-4">
              ...
              >
                {isGeneratingInvoice || isMultiQuotePending ? "Generating..." : "Generate invoice"}
              </ModernButton>
            </div>
          ) : (
        ...
        <ModernCard padding="lg" className="space-y-5">
          ...
          {showLeadOptions ? (
            <div className="space-y-4">
              ...
              >
                {isCreatingLead || isMultiQuotePending ? "Creating lead..." : "Create lead"}
              </ModernButton>
            </div>
          ) : (
            <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4 text-sm text-slate-500">
      ...
      <ModernCard padding="lg" variant="outlined" className="space-y-4">
        ...
        <ul className="grid grid-cols-1 gap-3 text-sm text-slate-600 md:grid-cols-2">
          ...
          <li className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <p className="mt-2">Push the calculation into the CRM when you capture lead details.</p>
          </li>
          <li className="rounded-2xl border border-slate-200 bg-white p-4">
