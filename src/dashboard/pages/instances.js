...
// Main Component
export default function TenantInstances() {
  ...
  const executeInstanceAction = async (instanceId, action) => {
    ToastUtils.warning(
      `Instance actions (${action}) are not yet available in the tenant dashboard. Please reach out to support for assistance.`
    );
  };
  ...
  const executeBulkAction = async (action) => {
    ...
    ToastUtils.warning(
      `Bulk actions (${action}) are not yet available in the tenant dashboard. Please manage instances individually for now.`
    );
    setSelectedInstances(new Set()); // Clear selection
  };

  // Navigate to instance details
  const navigateToInstanceDetails = (identifier) => {
    if (!identifier) {
      ToastUtils.warning("Instance identifier is missing.");
      return;
    }
    ToastUtils.info("Instance details view is coming soon.");
  };
  ...
  const headerActions = (
    <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
      ...
      <ModernButton
        variant="primary"
        size="sm"
        onClick={() => navigate("/dashboard/multi-instance-creation")}
