...
const DeleteTenantModal = ({ isOpen, onClose, tenantId, tenantName }) => {
    ...
    if (tenantId) {
      mutate(tenantId, {
        onSuccess: () => {
          ...
          // No need to manually refetch if react-query invalidates 'tenants' query key
        },
        onError: (err) => {
          console.error("Failed to delete Partner:", err);
          ToastUtils.error(
            err.message || "Failed to delete partner. Please try again."
          );
        },
      });
    } else {
      ...
      <div className="bg-white rounded-[24px] max-w-[500px] mx-4 w-full">
        ...
        <div className="px-6 py-6 w-full text-center">
          ...
          <p className="text-gray-700 mb-6">
            ...
            <strong className="text-gray-900">{tenantName || "N/A"}</strong>
