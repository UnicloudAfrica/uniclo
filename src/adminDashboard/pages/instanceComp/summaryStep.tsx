...
export const SummaryStep = ({ formData, pricingRequests }: any) => (
  <div className="space-y-6 w-full">
    <div className="text-center mb-6">
      <h3 className="text-xl font-semibold text-gray-800">Request Summary</h3>
      <p className="text-sm text-gray-500 mt-1">
        Please review the details of your instance request below before submitting.
      </p>
    </div>
    <div>
      ...
      <div className="p-4 bg-gray-50 rounded-lg space-y-2">
        ...
        <DetailRow label="Fast Track" value={formData.fast_track ? "Yes" : "No"} />
      </div>
    </div>
        ...
        {pricingRequests.length > 0 ? (
          pricingRequests.map((req, index) => (
            <div key={index} className="p-4 bg-gray-50 rounded-lg space-y-2">
              <h4 className="font-semibold text-gray-700">Configuration #{index + 1}</h4>
              ...
              {req.security_group_ids &&
                ...
                req.security_group_ids.length > 0 && (
                  <DetailRow label="Security Groups" value={req.security_group_ids.join(", ")} />
                )}

              <DetailRow label="Term" value={`${req.months} months`} />
              {req.bandwidth_id && (
                <DetailRow label="Bandwidth" value={`${req.bandwidth_count} units`} />
              )}
          ...
          <p className="text-sm text-gray-500">No configurations to summarize.</p>
        )}
      </div>
    </div>
  </div>
