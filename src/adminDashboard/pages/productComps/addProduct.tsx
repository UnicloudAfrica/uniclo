...
const AddProduct = ({ isOpen, onClose }: any) => {
  ...
  const enableProductFetch = !!selectedRegionCode && !!formData.productable_type;
  ...
  const objectStorageTotal = useMemo(() => {
    ...
    if (!Number.isFinite(quota) || quota <= 0 || !Number.isFinite(pricePerGb) || pricePerGb <= 0) {
      return 0;
    }
  ...
  const { isFetching: isVmsFetching, data: vms } = useFetchComputerInstances(
    ...
    selectedRegionCode,
    {
      enabled: enableProductFetch && formData.productable_type === "compute_instance",
    }
  );
  ...
  const { isFetching: isEbsVolumesFetching, data: ebsVolumes } = useFetchEbsVolumes(
    "USD",
    selectedRegionCode,
    {
      enabled: enableProductFetch && formData.productable_type === "volume_type",
    }
  );
  const { isFetching: isBandwidthsFetching, data: bandwidths } = useFetchBandwidths(
    "USD",
    selectedRegionCode,
    {
  ...
  const { isFetching: isIPsFetching, data: ips } = useFetchFloatingIPs("USD", selectedRegionCode, {
    enabled: enableProductFetch && formData.productable_type === "ip",
  });
  const { isFetching: isCrossConnectsFetching, data: crossConnects } = useFetchCrossConnect(
    "USD",
    selectedRegionCode,
    {
      enabled: enableProductFetch && formData.productable_type === "cross_connect",
    }
  );
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      setObjectStoragePricePerGb(DEFAULT_OBJECT_STORAGE_PRICE_PER_GB.toString());
    }
  }, [isOpen]);
  ...
  const validateForm = () => {
    ...
    if (!formData.productable_type) newErrors.productable_type = "Product type is required";
    if (isObjectStorage) {
      ...
      if (!Number.isFinite(priceValue) || priceValue <= 0) {
        newErrors.objectStoragePricePerGb = "Enter a valid price per GiB greater than zero.";
      }
  ...
  const handleSubmit = (e: any) => {
    ...
    if (isObjectStorage) {
      ...
      const providerResourceId =
        regionCode && quotaValue ? `object-storage-${regionCode}-${quotaValue}-gib` : null;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleSelectChange("productable_type", e.target.value)}
                  className={`w-full input-field ${
              ...
              {isObjectStorage ? (
                <>
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full input-field ${
                        errors.objectStorageQuota ? "border-red-500" : "border-gray-300"
                      }`}
                      disabled={isPending}
                    />
                    {errors.objectStorageQuota && (
                      <p className="text-red-500 text-xs mt-1">{errors.objectStorageQuota}</p>
                    )}
                  </div>
                  <div>
                    <label
                    ...
                    <input
                      ...
                      className={`w-full input-field ${
                        errors.objectStoragePricePerGb ? "border-red-500" : "border-gray-300"
                      }`}
                      disabled={isPending}
                    />
                    {errors.objectStoragePricePerGb && (
                      <p className="text-red-500 text-xs mt-1">{errors.objectStoragePricePerGb}</p>
                    )}
                  </div>
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-semibold text-slate-900">
                          {objectStorageTotal > 0 ? `$${objectStorageTotal.toFixed(4)}` : "—"}
                        </span>
                      </div>
                      <p className="mt-1 text-xs text-slate-500">
                        Calculated as quota × price per GiB. This value becomes the default regional
                        price.
                      </p>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => updateFormData("productable_id", e.target.value)}
                    className={`w-full input-field ${
                  ...
                  >
                    ...
                    {getProductOptions()?.map((product: any) => (
