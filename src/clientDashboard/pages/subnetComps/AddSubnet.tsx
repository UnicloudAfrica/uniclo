...
const AddSubnet: React.FC<AddSubnetProps> = ({ isOpen, onClose, projectId }: any) => {
  ...
  const { data: vpcs, isFetching: isFetchingVpcs } = useFetchClientVpcs(projectId);
  const { data: regions, isFetching: isFetchingRegions } = useFetchGeneralRegions();
  const { mutate: createSubnet, isPending: isCreating } = useCreateClientSubnet();
  ...
  const { data: existingSubnets, isFetching: isFetchingSubnets } = useFetchClientSubnets(
    projectId,
    formData.region,
    {
      enabled: !!projectId && !!formData.region,
  ...
  const { data: availableCidrs, isFetching: isFetchingAvailableCidrs } =
    useFetchAvailableClientCidrs(projectId, formData.region, formData.vpc_id, suggestPrefix, 8, {
  ...
  const validateForm = () => {
    ...
    const selectedVpc = (vpcs as any[])?.find((v) => String(v.id) === String(formData.vpc_id));
  ...
  const computeSuggestions = () => {
    ...
    try {
      ...
      for (let cur = alignedStart; cur + blockSize - 1 <= v.end; cur += blockSize) {
        ...
        const hasOverlap = used.some((r: any) => overlaps(cur, curEnd, r.start, r.end));
        if (!hasOverlap) {
