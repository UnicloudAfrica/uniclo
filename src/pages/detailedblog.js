    ...
    return (  
        ...
        <div className="mt-[8em] px-4 md:px-8 lg:px-16 w-full font-Outfit text-[#121212]">
          {selectedBlogItem && selectedBlogItem.date ? (
            <div>
              <p className="gradient-txt text-center font-medium text-base">{'Published ' + selectedBlogItem.date}</p>
              <p className="font-medium text-2xl md:text-[40px] md:leading-[50px] mt-3 md:px-[12%] text-center">{selectedBlogItem.title}</p>
              <p className="font-medium text-lg md:text-xl mt-3 md:px-[12%] text-center text-[#676767]">{selectedBlogItem.drawin}</p>
            </div>
          ) : null}
