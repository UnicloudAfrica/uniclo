...
import {
  ...
  useFetchClientEdgeNetworks,
  useFetchClientIpPools,
  useAssignClientProjectEdge,
...
const ClientAssignEdgeConfigModal = ({
  ...
  const { data: currentConfig } = useFetchClientProjectEdgeConfig(
    projectId,
    selectedRegion,
  ...
  } = useFetchClientEdgeNetworks(projectId, selectedRegion, {
    enabled: isOpen && !!selectedRegion,
  });
  ...
  const {
    ...
    {
      enabled:
        isOpen && !!selectedRegion && !!formData.edge_network_id,
    }
  );

  const { mutate: assignEdge, isPending } = useAssignClientProjectEdge();
  ...
  useEffect(() => {
    if (currentConfig && isOpen) {
      setFormData((prev) => ({
        ...
        ip_pool_id: currentConfig.edge_network_ip_pool_id || currentConfig.ip_pool_id || "",
  ...
  const handleRefreshFromProvider = async () => {
    try {
      ...
      console.warn("Refresh provider data failed", error);
    }
  };
          ...
          {currentConfig && (
            <div className="mb-4 w-full rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm text-gray-800">
              ...
              <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                <div>
                  Edge Network ID: {currentConfig.edge_network_id || "-"}
                </div>
                <div>
                  IP Pool ID:{" "}
            ...
            <div>
              ...
              {manualMode ? (
                ...
                >
                  ...
                  {(ipPools || []).map((pool) => (
                    <option
                      key={pool.id || pool.edge_network_ip_pool_id}
                      value={pool.id || pool.edge_network_ip_pool_id}
                    >
                      {pool.name || pool.label || pool.id}
                    </option>
                  ))}
                </select>
              )}
