...
export const AddAdminModal = ({ isOpen, onClose, mode = "modal" }: any) => {
  ...
  useEffect(() => {
    if (isError && error) {
      ToastUtils.error((error as { message?: string }).message || "Failed to add admin");
    }
  ...
  const handleSubmit = (e: any) => {
    ...
    createAdmin(adminData, {
      ...
      onError: (err) => {
        ToastUtils.error((err as { message?: string }).message || "Failed to add admin");
      },
    });
  };
  ...
  const summarySections = useMemo(
    ...
    [contactName, formData.email, formData.phone, formData.password, formData.password_confirmation]
  );
    ...
    <>
      <div className="rounded-2xl border border-slate-200 bg-white/80 p-5 shadow-sm backdrop-blur">
        ...
        <p className="mt-3 text-xs text-slate-500">
          Administrator accounts have full access to the control plane and should be provisioned
          carefully.
        </p>
      </div>
