...
export const EditAdminModal = ({ isOpen, onClose, admin, onUpdateSuccess, mode = "modal" }) => {
  ...
  const locationSnapshot = [formData.city, formData.state].filter(Boolean).join(", ");
  const meta = [
    {
      label: "Admin",
  ...
  const summarySections = useMemo(
    ...
    [formData.email, formData.phone, formData.role, formData.address, formData.city, formData.state]
  );
    ...
    <>
      <div className="rounded-2xl border border-slate-200 bg-white/80 p-5 shadow-sm backdrop-blur">
        ...
        <p className="mt-3 text-xs text-slate-500">
          Admin profiles should reflect the current responsibilities of the user to maintain proper
          access hygiene.
        </p>
      </div>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <header className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-500">Optional location details for audit records.</p>
            </div>
          </header>
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <div>
              <label htmlFor="country_id" className="mb-2 block text-sm font-medium text-slate-700">
                Country
              </label>
              <input
                type="text"
