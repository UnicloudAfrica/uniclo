...
import { useCreateKeyPair } from "@/shared/hooks/keyPairsHooks";
...
const AdminKeyPairCreate: React.FC = () => {
  ...
  useEffect(() => {
    if (urlProjectId && projects.length > 0) {
      ...
      const found = projects.find(
        (p: { id: string | number; identifier: string }) =>
          String(p.id) === urlProjectId || p.identifier === urlProjectId
      );
      if (found) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const project = projects.find((p: { id: number }) => p.id === Number.parseInt(projectId));
      ...
      logger.error(error);
      setError(error.message || "Failed to create key pair.");
    }
  };
            ...
            <div>
              ...
              <div className="relative">
                ...
                >
                  ...
                  {projects.map((project: { id: number; name: string; identifier: string }) => (
