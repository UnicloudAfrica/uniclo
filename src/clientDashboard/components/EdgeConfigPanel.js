...
import { useFetchClientProjectEdgeConfig } from "../../hooks/clientHooks/edgeHooks";
...
export default function EdgeConfigPanel({ projectId, region }) {
  const {
    ...
    error,
    refetch,
  } = useFetchClientProjectEdgeConfig(projectId, region, {
  ...
  return (
    <div className="bg-white rounded-[12px] p-6 shadow-sm mb-8">
      ...
      {isFetching ? (
        ...
        (!edgeConfig.edge_network_id && !edgeConfig.ip_pool_id) ? (
        <div className="flex items-start gap-3 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
          <AlertTriangle className="w-5 h-5 text-yellow-600 mt-0.5" />
          <div className="text-sm text-yellow-800">
            <p className="font-medium">No edge configuration assigned</p>
            <p>
              This project does not have an edge network and IP pool assigned
              yet. You may be unable to create subnets until this is configured.
            </p>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          ...
          <Field
            ...
            value={edgeConfig.flowlogs_enabled ? "Yes" : "No"}
          />
          {edgeConfig.metadata && (
