...
const SharedCreateInvoice = ({ mode = "admin", onExit }: SharedCreateInvoiceProps) => {
  ...
  // Hooks
  const { data: regions, isFetching: isRegionsFetching } = useSharedFetchRegions(mode) as {
  ...
  const validateItem = (): boolean => {
    ...
    if (!formData.storage_size_gb || Number(formData.storage_size_gb) < 1)
    ...
    if (validateItem()) {
      const computeName =
        computerInstances?.find(
          (c) => c.product.productable_id === Number(formData.compute_instance_id)
        )?.product.name || "N/A";
      const osName =
        osImages?.find((o) => o.product.productable_id === Number(formData.os_image_id))?.product
          .name || "N/A";

      const newRequest: PricingRequest = {
        region: formData.region,
        compute_instance_id: Number(formData.compute_instance_id),
        os_image_id: Number(formData.os_image_id),
        months: Number(formData.months),
        number_of_instances: Number(formData.number_of_instances),
        volume_types: [
          {
            volume_type_id: Number(formData.volume_type_id),
            storage_size_gb: Number(formData.storage_size_gb),
          },
        ],
        _display: {
          compute: computeName,
  ...
  const addObjectStorageRequest = () => {
    if (validateObjectStorageItem()) {
      ...
      const newRequest: ObjectStorageRequest = {
        ...
        months: Number(formData.object_storage_months),
        _display: {
          name: productName,
  ...
  const selectedTenant = tenants?.find(
    (tenant: { id: string | number }) => String(tenant.id) === String(selectedTenantId)
  );
  const selectedUser = userPool?.find(
    (user: { id: string | number }) => String(user.id) === String(selectedUserId)
  );
