...
const emptyCredentials: MspCredentials = {
  username: "",
  password: "",
  domain: "",
  domain_id: "",
...
const RegionRequestDetail = () => {
  ...
  const [region, setRegion] = useState<RegionRequest | null>(null);
  ...
  const handleVerifyCredentials = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (!id) {
        ToastUtils.error("Missing region request.");
        return;
      }
      setVerifying(true);
  ...
  const handleUpdateFulfillmentMode = async (mode: FulfillmentMode) => {
    try {
      if (!id) {
        ToastUtils.error("Missing region request.");
        return;
  ...
  const handleCancelRequest = async () => {
    if (!globalThis.window.confirm("Are you sure you want to cancel this region request?")) {
      return;
    }
  ...
  const getStatusBadge = (status: ApprovalStatus | string | null | undefined) => {
    const badges: Record<ApprovalStatus, { bg: string; text: string; label: string }> = {
    ...
    const resolvedStatus = status && status in badges ? (status as ApprovalStatus) : "pending";
  ...
  const credentialSummary: CredentialSummary = region?.msp_credential_summary ?? {};
      ...
      <TenantPageShell customHeader={<></>} disableContentPadding contentClassName="p-6 md:p-8">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
            ...
            <Link to="/dashboard/region-requests" className="text-blue-600 hover:text-blue-700">
              Back to regions
            </Link>
          </div>
        </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-green-600 mt-2">
                      Your MSP admin credentials authenticate using the default project token where
                      you have msp_admin role.
                    </p>
                  </div>
                </div>
                <button
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-yellow-600 mt-2">
                      Note: MSP admins authenticate using the default project token. Make sure you
                      have msp_admin role in your default project.
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <p className="text-xs text-blue-800">
                    ...
                    Ensure you have <strong>msp_admin</strong> role in your default project for
                    super-admin powers across all projects.
                  </p>
                </div>
              </div>
