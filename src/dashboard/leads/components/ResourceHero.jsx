...
const palette = {
  neutral: {
    container:
      "rounded-3xl border border-slate-200 bg-white shadow-sm text-slate-900",
    badge:
  ...
  midnight: {
    container:
      "rounded-3xl border border-slate-800 bg-slate-900 text-slate-100 shadow-lg",
    badge:
    ...
    metricLabel:
      "text-xs font-semibold uppercase tracking-wider text-slate-300/90",
    ...
    <section className={theme.container}>
      <div className="flex flex-col gap-6 px-6 py-8 sm:px-10 sm:py-10 lg:flex-row lg:items-start lg:justify-between">
        <div className="max-w-2xl space-y-3">
          {subtitle ? <span className={theme.badge}>{subtitle}</span> : null}
      ...
      {metrics.length ? (
        <div className="border-t border-slate-100 px-6 pb-8 pt-6 sm:px-10">
