...
const Vms = ({ selectedRegion, onMetricsChange }) => {
  ...
  const avgMemory = useMemo(() => {
    ...
    const sum = rows.reduce(
      (acc, vm) => acc + Number(vm.memory_mb || 0),
      0
    );
  ...
  const emptyState = {
    ...
    description:
      "Define compute templates so tenants can provision workloads in this region.",
    action: (
