...
const PricingWorkloadCard = ({
  ...
  const validateVolumeInput = () => {
    ...
    if (!data.storage_size_gb || data.storage_size_gb < 1) newErrors.storage_size_gb = "Min 1 GB";
  ...
  const addVolume = () => {
    ...
    const volProduct = ebsVolumes?.find(
      (v) => String(v.product.productable_id) === String(data.volume_type_id)
    );
    ...
    <ModernCard padding="none" className="overflow-hidden border border-slate-200 shadow-sm">
      ...
      >
        ...
        <div className="flex items-center gap-4">
          {!isExpanded && (
            <span className="text-sm font-medium text-slate-900">{calculateEstimatedPrice()}</span>
          )}
          {isExpanded ? (
            ...
            <div className="space-y-6 p-6 border-t border-slate-200 bg-white">
              ...
              <div className="border-t border-slate-200 pt-4">
                ...
                >
                  ...
                  {isNetworkingOpen
                    ? "Hide Networking Options"
                    : "Show Networking Options (Optional)"}
                </button>
                    ...
                    >
                      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <SelectableInput
                          ...
                          onSearchChange={(v) =>
                            setSearchTerms((prev) => ({ ...prev, bandwidth: v }))
                          }
                        ...
                        <SelectableInput
                          ...
                          onSearchChange={(v) =>
                            setSearchTerms((prev) => ({ ...prev, floatingIp: v }))
                          }
