...
const IGWs = ({ projectId = "", region = "" }: any) => {
  const selectedRegion = region;
  const queryClient = useQueryClient();
  const { data: igws, isFetching: isFetchingIgws } = useFetchIgws(projectId, selectedRegion, {
  ...
  const openDeleteModal = (igw: any) => setDeleteModal({ igw });
  const closeDeleteModal = () => setDeleteModal(null);
  const openViewModal = (igw: any) => setViewModal(igw);
  const closeViewModal = () => setViewModal(null);
  const openAttachModal = (igw: any, mode: any) => setAttachModal({ igw, mode });
  ...
  const renderGatewayCard = (igw: any) => {
    const displayName =
