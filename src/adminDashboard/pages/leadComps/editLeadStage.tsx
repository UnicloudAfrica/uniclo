...
import { useUpdateLeadStage } from "@/hooks/adminHooks/leadsHooks";
...
export const EditLeadStage = ({ isOpen, onClose, stage, lead }: { isOpen: boolean; onClose: () => void; stage: unknown; lead: unknown }) => {
  ...
  const formatStatusForDisplay = (status: unknown) => {
    return status.replace(/_/g, " ").replace(/\b\w/g, (c: unknown) => c.toUpperCase());
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1000] font-Outfit">
      <div className="bg-white rounded-[24px] max-w-[650px] mx-4 w-full">
        <div className="flex justify-between items-center px-6 py-4 border-b bg-[var(--theme-surface-alt)] rounded-t-[24px] w-full">
          <h2 className="text-lg font-semibold text-[var(--theme-text-color)]">Edit Lead Stage</h2>
          <button
            onClick={onClose}
          ...
          <div className="space-y-4 w-full">
            <div>
              ...
              >
                ...
                {leadStatusOptions.map((status: unknown) => (
            ...
            <div>
              ...
              >
                ...
                {isAdminsLoading ? (
                  ...
                  (admins as Record<string, unknown>[])?.map((admin) => (
                    <option key={admin.id as string} value={admin.identifier as string}>
