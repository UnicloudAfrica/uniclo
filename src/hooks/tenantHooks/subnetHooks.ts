...
const deleteTenantSubnet = async ({
  id,
  payload,
}: {
  id: string;
...
export const useFetchTenantSubnetById = (id: string, options: Record<string, unknown> = {}) =>
  useQuery({
...
export const syncTenantSubnetsFromProvider = async ({
  project_id,
  region,
}: {
  project_id: string;
