...
const AddKeyTenantPair = ({ isOpen, onClose, projectId = "", region = "" }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const keyPairData = {
      ...
      region: region || formData.region, // Use passed region or form data
      name: formData.name,
  ...
  const handleDone = () => {
    ...
    // Invalidation is handled by the mutation's onSuccess
    onClose();
  };
          ...
          <div className="space-y-4 w-full">
            {successState.isSuccess ? (
              ...
              <>
                ...
                {!region && ( // Only show region selector if not passed as a prop
                  <div>
                    <label
                      htmlFor="region"
                      className="block text-sm font-medium text-gray-700 mb-2"
