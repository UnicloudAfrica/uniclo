...
import {
  useAttachClientNetworkInterfaceSecurityGroup,
  useDetachClientNetworkInterfaceSecurityGroup,
...
export const AttachSgModal = ({
  ...
  const { mutate: attach, isPending } =
    useAttachClientNetworkInterfaceSecurityGroup();
  const { data: securityGroups } = useFetchClientSecurityGroups(
    projectId,
    form.region,
  ...
  const submit = (e) => {
    ...
    if (!form.security_group_id)
      eobj.security_group_id = "Security Group ID is required";
    if (!form.network_interface_id)
      eobj.network_interface_id = "ENI ID is required";
    setErrors(eobj);
    ...
    attach(
      {
        ...
        region: form.region,
        network_interface_id: form.network_interface_id,
      ...
      },
      {
        ...
        onError: (err) =>
          ToastUtils.error(
            err?.message || "Failed to attach security group"
          ),
      }
    );
  };
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({
                  ...p,
                  network_interface_id: e.target.value,
                }))
              }
              className={`w-full border rounded px-3 py-2 text-sm ${
  ...
  const submit = (e) => {
    ...
    detach(
      ...
      },
      {
        ...
        onError: (err) =>
          ToastUtils.error(
            err?.message || "Failed to detach security group"
          ),
      }
    );
  };
