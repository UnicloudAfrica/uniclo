...
const AddEniModal = ({ isOpen, onClose, projectId, region: defaultRegion = "" }) => {
  ...
  const { mutate: createEni, isPending } = useCreateTenantNetworkInterface();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1000] font-Outfit">
      <div className="bg-white rounded-[24px] max-w-[540px] w-full mx-4">
        <div className="flex items-center justify-between px-6 py-4 border-b bg-[#F2F2F2] rounded-t-[24px]">
          <h2 className="text-lg font-semibold text-[#575758]">Create Network Interface</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setForm((prev) => ({ ...prev, network_id: e.target.value }))}
