...
import {
  useFetchClientRouteTables,
  useCreateClientRouteTableAssociation,
  useDeleteClientRoute,
  syncClientRouteTablesFromProvider,
} from "../../../hooks/clientHooks/routeTableHooks";
...
const formatAssociationLabel = (assoc: any): string => {
  if (assoc == null) {
    return "Unknown";
  }
...
const RouteTables: React.FC<RouteTablesProps> = ({ projectId = "", region = "" }) => {
  const queryClient = useQueryClient();
  const { data: routeTables, isFetching } = useFetchClientRouteTables(projectId, region);
  const { mutate: associateRouteTable, isPending: associating } =
    useCreateClientRouteTableAssociation();
  const { mutate: deleteRoute } = useDeleteClientRoute();
  ...
  const { data: subnets } = useFetchClientSubnets(projectId, region, {
    enabled: !!projectId && !!region,
  });
  const { data: igws } = useFetchClientIgws(projectId, region, {
    enabled: !!projectId && !!region,
  });

  // Use Record<string, string> for igwChoice
  const [igwChoice, setIgwChoice] = useState<Record<string, string>>({});
  const [isSyncing, setIsSyncing] = useState(false);

  const tables = useMemo<RouteTableType[]>(() => (routeTables as any[]) || [], [routeTables]);
  ...
  const handleSync = async () => {
    ...
    setIsSyncing(true);
    try {
      await syncClientRouteTablesFromProvider({
        project_id: projectId,
        region,
      });
