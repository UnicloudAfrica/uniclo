...
const CalculatorConfigStep = ({
  ...
  const validateCurrentItem = () => {
    ...
    if (!currentItem.compute_instance_id) newErrors.compute_instance_id = "Compute instance is required.";
    if (!currentItem.os_image_id) newErrors.os_image_id = "OS image is required.";
    if (!currentItem.months || currentItem.months < 1) newErrors.months = "Term must be at least 1 month.";
    if (!currentItem.number_of_instances || currentItem.number_of_instances < 1) newErrors.number_of_instances = "At least 1 instance is required.";
    if (!currentItem.volume_type_id) newErrors.volume_type_id = "Volume type is required.";
    if (!currentItem.storage_size_gb || currentItem.storage_size_gb < 1) newErrors.storage_size_gb = "Storage size must be at least 1 GB.";
  ...
  const addCurrentItem = () => {
    if (validateCurrentItem()) {
      ...
      const newRequest = {
        ...
        cross_connect_id: currentItem.cross_connect_id ? parseInt(currentItem.cross_connect_id) : null,
        _display: {
          compute: computeName,
          os: osName,
  ...
  return (
    <div className="space-y-6 w-full max-w-4xl">
      ...
      {/* Total Discount Configuration */}
      <div className="bg-green-50 p-4 rounded-lg">
        <h4 className="font-semibold text-gray-800 mb-3">Total Order Discount</h4>
        <div className="space-y-3">
          <div className="flex items-center space-x-2">
            ...
            <label htmlFor="apply_total_discount" className="text-sm text-gray-700">
              Apply discount to entire calculation
            </label>
          </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <input
                  ...
                  className={`${inputClass} ${errors.total_discount_value ? "border-red-500" : ""}`}
                  placeholder={calculatorData.total_discount_type === 'percent' ? '10' : '100.00'}
                />
                {errors.total_discount_value && (
                  <p className="text-red-500 text-xs mt-1">{errors.total_discount_value}</p>
                )}
              </div>
      ...
      <div className="bg-gray-50 p-6 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            {isComputerInstancesFetching ? (
              ...
              <select
                ...
                className={`${inputClass} ${itemErrors.compute_instance_id ? "border-red-500" : ""}`}
            ...
            {itemErrors.compute_instance_id && <p className="text-red-500 text-xs mt-1">{itemErrors.compute_instance_id}</p>}
          </div>
          ...
          <div>
            ...
            {itemErrors.os_image_id && <p className="text-red-500 text-xs mt-1">{itemErrors.os_image_id}</p>}
          </div>
          ...
          <div>
            ...
            />
            {itemErrors.number_of_instances && <p className="text-red-500 text-xs mt-1">{itemErrors.number_of_instances}</p>}
          </div>
          ...
          <div>
            ...
            {itemErrors.volume_type_id && <p className="text-red-500 text-xs mt-1">{itemErrors.volume_type_id}</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="100"
            />
            {itemErrors.storage_size_gb && <p className="text-red-500 text-xs mt-1">{itemErrors.storage_size_gb}</p>}
          </div>
          ...
          <div>
            ...
            >
              ...
              {bandwidths?.map((bandwidth) => (
                <option key={bandwidth.product.productable_id} value={bandwidth.product.productable_id}>
        ...
        <div className="mt-4">
          <button
            ...
            className="px-4 py-2 bg-[#288DD1] text-white rounded-md hover:bg-[#1976D2] transition-colors flex items-center"
          >
              ...
              <div key={index} className="bg-white p-4 rounded-lg border border-gray-200 flex justify-between items-center">
                <div>
                  ...
                  <div className="text-sm text-gray-600">
                    OS: {request._display?.os || "N/A"} • Region: {request.region} • Term: {request.months} month(s)
                  </div>
                </div>
                <button
