...
const DeleteObjectStorageTierModal = ({ isOpen, onClose, tier, onDeleted }) => {
  const { mutate: deleteProduct, isPending: isDeletingProduct } = useDeleteProduct();
  const {
    mutate: deletePricing,
    isPending: isDeletingPricing,
  } = useDeleteProductPricing();
      ...
      <div className="w-full max-w-[500px] rounded-[24px] bg-white shadow-xl">
        ...
        <div className="space-y-6 px-6 py-6 text-sm text-slate-600">
          ...
          <p className="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 text-xs text-slate-500">
            This will delete the SKU from inventory along with its pricing entry.
            Tenants who rely on this tier will no longer see it available for orders
            until you re-add it.
          </p>
        </div>
