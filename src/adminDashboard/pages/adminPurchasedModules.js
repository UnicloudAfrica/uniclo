...
export default function AdminPurchasedModules() {
  ...
  const moduleStats = {
    ...
    recentPurchases: purchasedProducts?.filter(module => {
      const purchaseDate = new Date(module.purchasedDate || module.created_at);
      const thirtyDaysAgo = new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
      return purchaseDate >= thirtyDaysAgo;
    }).length || 0,
    uniqueModuleTypes: [...new Set(purchasedProducts?.map(module => module.type || 'Standard') || [])].length
  };
          ...
          <ModernCard className="max-w-xl w-full">
            <div className="text-center">
              ...
              <div className="flex items-center justify-center space-x-2">
                <ModernButton
                  onClick={() => refetchPurchasedProducts()}
                  variant="outline"
                >
                  Retry
                </ModernButton>
                <ModernButton onClick={handleAddModule}>
                  Add Module
                </ModernButton>
              </div>
            </div>
