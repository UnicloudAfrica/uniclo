...
const FileDropInput = ({
  ...
  const renderPreview = () => {
    ...
    if (typeof value === "string") {
      ...
      if (value.startsWith("data:")) {
        return (
          <span className="text-xs text-emerald-600 text-center">New file ready to upload.</span>
        );
      }
    ...
    if (typeof value === "object" && value !== null) {
      ...
      const label =
        value.original_name ?? value.name ?? value.path?.split("/").pop() ?? "Uploaded file";
      ...
      return (
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-xs text-gray-600 text-center break-all">{label}</span>
        </div>
      );
    }
  ...
  return (
    <div className="space-y-2">
      ...
      {helperText && <p className="text-xs text-gray-500 leading-snug">{helperText}</p>}
      <input
        ref={inputRef}
        type="file"
        accept={accept}
