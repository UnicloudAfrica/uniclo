...
const ADMIN_PERSONA_OPTIONS: PersonaOption[] = [
  {
...
const OnboardingReviewPage: React.FC<OnboardingReviewPageProps> = ({
  ...
  const loadStepSummaries = useCallback(async () => {
    ...
    try {
      ...
      ToastUtils.error(error?.message ?? "Failed to load onboarding submissions.");
    } finally {
  ...
  const subjectLabel = useMemo(() => {
    ...
    if (target === "tenant") {
      return (
        tenantsOptions.find((option: any) => option.value === tenantId)?.label ?? tenantId ?? "—"
      );
    }

    return filteredClients.find((option: any) => option.value === userId)?.label ?? userId ?? "—";
  ...
  const renderPersonaOption = (option: PersonaOption) => {
