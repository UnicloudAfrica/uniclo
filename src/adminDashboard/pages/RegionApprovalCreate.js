...
const RegionApprovalCreate = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      if (name === "ownership_type") {
        ...
        } else if (value === "tenant_owned" && (!prev.platform_fee_percentage || prev.platform_fee_percentage === "0")) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setSubmitting(true);
      const sanitize = (value, transformer = (v) => v) => {
  ...
  const backHref = isTenantOwned
    ? "/admin-dashboard/region-approvals"
    : "/admin-dashboard/regions";
  ...
  const tenantOptions = useMemo(
    () =>
      tenants.map((tenant) => ({
        value: tenant.id,
        label:
          tenant.name || tenant.identifier || tenant.slug || tenant.email || tenant.id,
      })),
    [tenants]
  );
  ...
  const selectedTenantName = selectedTenant
    ? selectedTenant.name || selectedTenant.identifier || selectedTenant.slug || selectedTenant.email || selectedTenant.id
    : "—";
  ...
  const infoBanner = useMemo(() => {
    if (!providerIsZadara) {
      return {
        ...
        title: "Coming soon",
        body:
          "Automated provisioning is currently available for Zadara only. Regions on other providers will operate in manual mode until integrations are released.",
      };
    }
    ...
    const base =
      formData.fulfillment_mode === "automated"
        ? {
            ...
            body:
              "This region will provision resources automatically using MSP admin credentials. Ensure credentials are verified in the region detail screen.",
          }
        : {
            tone: "warning",
            title: "Manual fulfillment",
            body:
              "Orders in this region will require manual intervention. MSP credentials are optional but recommended for consistency.",
          };
...
const renderInput = (id, label, placeholder, Icon, props = {}) => (
  <div className="space-y-2">
    <label htmlFor={id} className="text-sm font-medium text-gray-700">
      ...
      <div
        className={`flex items-center gap-3 rounded-lg border px-3 py-2 focus-within:ring-2 ${errors[id]
  ...
  const renderObjectStorageSection = () => (
    <ModernCard title="Object Storage (Optional)" className="space-y-4">
      <div className="flex flex-col gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-semibold text-gray-800">Enable Zadara Object Storage for this region?</p>
          <p className="mt-1 text-xs text-gray-600">
            Turning this on lets tenants in this region receive S3 credentials managed by the CRM. You&apos;ll need a valid access key.
          </p>
        </div>
    ...
    <>
      ...
      >
        ...
        <ModernCard padding="lg" className="space-y-6">
          <div
            className={`flex items-start gap-3 rounded-xl border px-4 py-3 text-sm ${infoBanner.tone === "warning"
              ...
              <div className="space-y-2">
                ...
                {!providerIsZadara && formData.provider && (
                  <p className="text-xs text-amber-600">
                    Automated provisioning for {formData.provider.toUpperCase()} is coming soon. Regions will operate in manual mode until the integration is ready.
                  </p>
                )}
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  {!providerIsZadara && (
                    <p className="text-xs text-gray-500">
                      Base URL will be captured once automated provisioning is supported for this provider.
                    </p>
                  )}
                </div>
              </div>
                  ...
                  {[
                    {
                      ...
                      description:
                        "Provision orders automatically using MSP admin credentials.",
                    },
                    {
                      value: "manual",
                      title: "Manual",
                      description:
                        "Admin manually processes each order created in this region.",
                    },
                  ].map((mode) => (
                    <label
                      key={mode.value}
                      ...
                      <div>
                        ...
                        {mode.value === "automated" && !providerIsZadara && (
                          <p className="text-[11px] text-amber-600 mt-1">
                            Enable automated fulfilment once {formData.provider.toUpperCase()} integration is live.
                          </p>
                        )}
                      </div>
                    </label>
...
const FeeField = ({ value, onChange, error }) => (
  <div className="space-y-2">
