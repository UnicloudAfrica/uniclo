...
const getToneForStatus = (status = "") => {
  ...
  if (["available", "active", "in-use", "inuse"].includes(normalized))
    return "success";
...
const ENIs = ({ projectId = "", region = "" }) => {
  const { data: enis, isFetching } = useFetchClientNetworkInterfaces(
    projectId,
    region
  );
        ...
        <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
          {list.map((eni) => {
            ...
            const primaryIp =
              ...
              "—";
            const macAddress =
              record?.mac_address || record?.mac || eni.mac_address || null;
            const zone =
              record?.availability_zone || eni.availability_zone || null;
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          <ModernButton
                            ...
                            onClick={() =>
                              setAttachModal({ open: true, eniId: id })
                            }
                          >
                            Attach
                          </ModernButton>
                          <ModernButton
                            ...
                            onClick={() =>
                              setDetachModal({ open: true, eniId: id })
                            }
                        ...
                        <div className="flex flex-wrap gap-2">
                          {securityGroups.map((sg, idx) => {
                            const label =
                              typeof sg === "string"
                                ? sg
                                : sg.id || sg.name || JSON.stringify(sg);
                            return (
                              <span
                                key={`${id}-sg-${idx}`}
