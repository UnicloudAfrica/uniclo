...
const AddProductPricing = ({ isOpen, onClose }: any) => {
  ...
  const handlePricingChange = (index, field, value) => {
    ...
    if (field === "category") {
      ...
      updatedPricing[field] = field === "price_usd" ? parseFloat(value) || "" : value;
    }
  ...
  const removePricingEntry = (index: any) => {
  ...
  const productCategories = useMemo(() => {
    if (!products) return [];
    const categories = new Set(products.map((p: any) => p.productable_type));
  ...
  const handleSubmit = (e: any) => {
    ...
    const payload = {
      ...
      pricings: formData.pricings.map(({ product_id, price_usd }: any) => ({
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1000] font-Outfit">
      <div className="bg-white rounded-[24px] max-w-[650px] mx-4 w-full">
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b bg-[#F2F2F2] rounded-t-[24px] w-full">
          <h2 className="text-lg font-semibold text-[#575758]">Add Product Pricing</h2>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 w-full overflow-y-auto flex flex-col items-center max-h-[400px] justify-start">
          <div className="space-y-4 w-full">
            {productErrors && <p className="text-red-500 text-sm">{productErrors.message}</p>}
            {/* Country Field */}
            <div>
              <label
                  ...
                  <div>
                    ...
                    <div className="w-full">
                      ...
                      >
                        ...
                        {products
                          ?.filter((p) => p.productable_type === pricing.category)
                          .map((product: any) => (
