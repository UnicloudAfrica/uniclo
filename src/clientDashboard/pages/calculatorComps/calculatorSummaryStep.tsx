...
const CalculatorSummaryStep = ({ calculatorData, pricingResult, onRecalculate }) => {
    ...
    try {
      const payload = {
        ...
        lead_info: {
          ...
          last_name: invoiceData.bill_to_name
            ? invoiceData.bill_to_name.split(" ").slice(1).join(" ") || ""
            : "",
  ...
  const storageSubtotal = hasApiStorage
    ...
    : storageLines.reduce((sum, line) => sum + Number(line.total || 0), 0);

  const baseSubtotal = Number(pricing.subtotal || 0);
  const baseTax = Number(pricing.tax || 0);
  const basePreDiscount = Number(pricing.pre_discount_subtotal || baseSubtotal);
  ...
  if (hasApiStorage) {
    combinedLines = Array.isArray(pricing.lines) ? pricing.lines.map(normalizeLine) : storageLines;
    ...
    combinedLines = [
      ...(Array.isArray(pricing.lines) ? pricing.lines.map(normalizeLine) : []),
      ...storageLines,
    ];
    ...
    combinedTotal = Number(pricing.total || 0) + storageSubtotal + storageTax;
  }

  const displayCurrency =
    pricing.currency || storageLines[0]?.currency || calculatorData.currency_code || "USD";
  ...
  const combinedRegions = hasApiStorage
    ...
    : new Set([...(summary.regions || []), ...storageItems.map((item: any) => item.region)]);
  const showPreDiscountRow = basePreDiscount !== baseSubtotal || storageSubtotal > 0;
  ...
  return (
    <div className="space-y-6 w-full max-w-4xl">
      ...
      <div className="bg-white border rounded-lg p-6">
        ...
        <div className="mb-4">
          <ModernTable
            data={combinedLines.map((line, idx) => ({ ...line, id: idx }))}
            columns={[
              {
                key: "name",
                header: "ITEM",
                render: (_, line) => (
                  <div>
                    ...
                    {line.term_months && (
                      <div className="text-xs text-gray-500">
                        Term: {line.term_months} month{line.term_months === 1 ? "" : "s"}
                      </div>
                    )}
                  </div>
                ),
              ...
              },
              {
                ...
                header: "UNIT PRICE",
                align: "right",
                render: (val, line) => formatCurrency(val, line.currency || displayCurrency),
              },
              {
                key: "total",
                header: "TOTAL",
                ...
                render: (val, line) => (
                  <span className="font-semibold">
                    {formatCurrency(val, line.currency || displayCurrency)}
                  </span>
                ),
              },
            ]}
        ...
        <div className="mt-6 grid grid-cols-2 md:grid-cols-5 gap-4 bg-gray-50 p-4 rounded-lg">
          <div>
            ...
            <div className="text-lg font-semibold text-gray-900">{combinedTotalItems}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="text-lg font-semibold text-purple-600">{combinedRegions.size}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="text-lg font-semibold text-gray-900">{displayCurrency}</div>
          </div>
        </div>
      </div>
