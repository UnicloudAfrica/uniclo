...
export const ResourceSection: React.FC<ResourceSectionProps> = ({
  ...
  return (
    <ModernCard variant="outlined" padding="lg" className="w-full space-y-6 overflow-hidden">
          ...
          <div className="flex w-full flex-wrap items-center justify-start gap-2 md:w-auto md:justify-end">
            {Array.isArray(actions)
              ? actions.map((action, index) => {
                  // If it's a valid element, try to use its key, otherwise fallback to index
                  const key = React.isValidElement(action) ? action.key : index;
                  return (
                    <div key={key || index} className="flex-shrink-0">
                      {action}
                    </div>
                  );
                })
