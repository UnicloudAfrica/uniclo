...
const BusinessInfo = ({
  ...
  const handleVerifyBusiness = () => {
    if (
      ...
      ToastUtils.error(
        "Please fill in all required business fields to verify."
      );
      return;
    }
    ...
    verifyBusiness(verificationData, {
      onSuccess: (data) => {
        ...
        setFormData((prev) => ({
          ...prev,
          verification_token:
            data?.verification_token || prev.verification_token,
...
BusinessInfo.validate = (formData) => {
  ...
  if (
    ...
    !/^\+?\d{10,15}$/.test(formData.business.phone)
  )
