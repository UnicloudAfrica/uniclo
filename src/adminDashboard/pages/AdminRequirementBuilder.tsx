 ...
 * Plain language + a live preview; all the technical knobs (key, placement,
 * scope, grace) are auto-defaulted and tucked under "Advanced".
 */

// The kinds of question, in friendly words. (These map to the renderer's field types.)
...
// Sub-questions inside a repeatable section can't themselves repeat or be an ID check.
...
// ── A guided "step" card with a big number ───────────────────────────────────
...
const AdminRequirementBuilder = () => {
  ...
  if (isEdit && !seeded) {
    if (!existing) {
      ...
      return (
        <AdminPageShell title="Form not found" description="This form may have been removed.">
  ...
  // ── Serialize → API payload (auto-derive + dedupe field keys) ───────────────
  ...
  // ── Live preview (what the person will see) ─────────────────────────────────
  ...
  return (
    <AdminPageShell
      title={isEdit ? "Edit form" : "Build a form"}
      description="Create the form people fill in. Add your questions, choose who it’s for, and watch the live preview."
      breadcrumbs={[
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-100 p-4">
              <div className="rounded-xl bg-white p-5 shadow-sm">
                ...
                <button
                  type="button"
                  disabled
                  className="mt-5 w-full rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white opacity-90"
                >
                  Continue
                </button>
              </div>
