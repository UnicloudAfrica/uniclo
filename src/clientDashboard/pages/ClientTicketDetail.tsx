...
const ClientTicketDetail: React.FC = () => {
  ...
  const rateMutation = useMutation({
    mutationFn: (payload: { score: number; comment?: string }) =>
      clientApi("POST", `/business/support/${id}/rate`, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["client", "support", "detail", id] });
    },
  });
  ...
  const markRead = (messageId: number) => {
    ...
    return clientSilentApi("POST", `/business/support/${id}/read`, {
        ...
        >
          <div className="text-center py-12">
            ...
            <p className="mt-2 text-sm text-gray-500">
              The ticket you are looking for does not exist or you do not have permission to view
              it.
            </p>
          </div>
        </ClientPageShell>
      ...
      >
        <SharedTicketDetail
          ...
          onFetchMessages={(page) =>
            clientSilentApi("GET", `/business/support/${id}/messages?per_page=15&page=${page}`)
          }
