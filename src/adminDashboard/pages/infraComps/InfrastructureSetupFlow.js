...
import { CheckCircle, AlertCircle, Loader2, ArrowRight, ExternalLink, RefreshCw } from 'lucide-react';
import { 
...
const StepIcon = ({ status, isLoading }) => {
  if (isLoading) {
...
const ActionButton = ({ onClick, disabled, isLoading, children, variant = 'primary' }) => {
  const baseClasses = "px-6 py-2 rounded-lg font-medium text-sm transition-all duration-200 flex items-center gap-2";
  const variants = {
    primary: "bg-blue-500 hover:bg-blue-600 text-white disabled:bg-gray-300 disabled:cursor-not-allowed",
    secondary: "bg-gray-100 hover:bg-gray-200 text-gray-700 disabled:bg-gray-50 disabled:cursor-not-allowed",
    danger: "bg-red-500 hover:bg-red-600 text-white disabled:bg-gray-300 disabled:cursor-not-allowed"
  };
...
const InfrastructureStep = ({ 
  ...
  const getStatusColor = () => {
    switch (status) {
      case StepStatus.COMPLETED: return 'border-green-200 bg-green-50';
      case StepStatus.ERROR: return 'border-red-200 bg-red-50';
      case StepStatus.IN_PROGRESS: return 'border-blue-200 bg-blue-50';
      default: return 'border-gray-200 bg-white';
    }
  };

  return (
    <div className={`p-4 rounded-lg border-2 transition-all duration-200 ${getStatusColor()} ${isActive ? 'ring-2 ring-blue-300' : ''}`}>
          ...
          <div className="flex-1">
            ...
            {details && (
              <div className="text-xs text-gray-500 bg-gray-100 p-2 rounded mb-2">
                <pre className="whitespace-pre-wrap font-mono">{JSON.stringify(details, null, 2)}</pre>
              </div>
            )}
...
const InfrastructureSetupFlow = ({ projectId, projectName }) => {
  ...
  const completedSteps = infraStatus?.components ? 
    Object.values(infraStatus.components).filter(comp => comp.status === 'completed').length : 0;
  const totalSteps = infrastructureSteps.length;
  const progress = totalSteps > 0 ? (completedSteps / totalSteps) * 100 : 0;

  // Handle component setup
  const handleSetupComponent = (componentType) => {
  ...
  const canStepProceed = (stepIndex) => {
