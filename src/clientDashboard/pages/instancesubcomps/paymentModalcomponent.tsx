...
const PaymentModal = ({ isOpen, onClose, transaction, onPaymentInitiated }: any) => {
  ...
  useEffect(() => {
    if (isOpen && transaction?.payment_gateway_options?.length > 0) {
      const paystackCardOption = transaction.payment_gateway_options.find(
        (option) =>
          option.name.toLowerCase() === "paystack" && option.payment_type.toLowerCase() === "card"
      );
      setSelectedPaymentOption(paystackCardOption || transaction.payment_gateway_options[0]);
    } else {
  ...
  const handlePaystackCardPayment = useCallback(() => {
    if (!paystackKey || !profile?.email || !transaction?.amount || !transaction?.identifier) {
  ...
  }, [paystackKey, profile?.email, transaction, saveCard, popup, onClose, onPaymentInitiated]);
  ...
  const computeCost =
    Number(pricingBreakdown?.lines?.find((line) => line.slug === "compute")?.total_local) || 0;
  const storageCost =
    Number(pricingBreakdown?.lines?.find((line) => line.slug === "storage")?.total_local) || 0;
  const osCost =
    Number(pricingBreakdown?.lines?.find((line) => line.slug === "os-image")?.total_local) || 0;
  const bandwidthCost =
    Number(pricingBreakdown?.lines?.find((line) => line.slug === "bandwidth")?.total_local) || 0;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1010] font-Outfit">
      <div className="bg-white rounded-[24px] max-w-[650px] mx-4 w-full">
        <div className="flex justify-between items-center px-6 py-4 border-b bg-[#F2F2F2] rounded-t-[24px] w-full">
          <h2 className="text-lg font-semibold text-[#575758]">Complete Payment</h2>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 w-full overflow-y-auto flex flex-col items-center max-h-[400px] justify-start">
          <div className="space-y-6 w-full">
            <div>
              ...
              <span className="w-full px-2 py-4 border border-[#E9EAF4] rounded-[10px] block">
                ...
                transaction.payment_gateway_options.length > 0 ? (
                  ...
                  >
                    {transaction.payment_gateway_options.map((option: any) => (
            ...
            <div className="bg-[#F8F8F8] rounded-lg py-4 px-6 text-left">
              <h3 className="text-sm font-medium text-[#1C1C1C] mb-4">Transaction Breakdown</h3>
              {pricingBreakdown ? (
                <>
                  <div className="flex w-full items-center justify-between mb-2">
                    <span className="text-sm font-normal text-[#676767]">Compute Cost:</span>
                    <span className="text-sm font-normal text-[#1c1c1c]">
                      {pricingBreakdown.currency} {Number(computeCost).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex w-full items-center justify-between mb-2">
                    <span className="text-sm font-normal text-[#676767]">Storage Cost:</span>
                    <span className="text-sm font-normal text-[#1c1c1c]">
                      {pricingBreakdown.currency} {Number(storageCost).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex w-full items-center justify-between mb-2">
                    <span className="text-sm font-normal text-[#676767]">OS Cost:</span>
                    <span className="text-sm font-normal text-[#1c1c1c]">
                      {pricingBreakdown.currency} {Number(osCost).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex w-full items-center justify-between mb-2">
                    <span className="text-sm font-normal text-[#676767]">Bandwidth Cost:</span>
                    <span className="text-sm font-normal text-[#1c1c1c]">
                      {pricingBreakdown.currency} {Number(bandwidthCost).toLocaleString()}
                    </span>
                  </div>
                  <hr className="my-2 border-[#E9EAF4]" />
                  <div className="flex w-full items-center justify-between mb-2">
                    <span className="text-sm font-medium text-[#676767]">Subtotal:</span>
                    <span className="text-sm font-normal text-[#1c1c1c]">
                  ...
                  <div className="flex w-full items-center justify-between mb-2">
                    <span className="text-sm font-medium text-[#676767]">
                      Tax (
                      {Number((pricingBreakdown.tax / pricingBreakdown.subtotal) * 100).toFixed(2)}%
                      VAT):
                    </span>
                    <span className="text-sm font-normal text-[#1c1c1c]">
                      {pricingBreakdown.currency} {Number(pricingBreakdown.tax).toLocaleString()}
                    </span>
                  </div>
                  <hr className="my-2 border-[#E9EAF4]" />
                  <div className="flex w-full items-center justify-between font-semibold">
                    ...
                    <span className="text-sm text-[#1c1c1c]">
                      {pricingBreakdown.currency} {Number(pricingBreakdown.total).toLocaleString()}
                    </span>
                  </div>
                  <div className="flex w-full items-center justify-between font-semibold mt-2">
                    ...
                    <span className="text-sm text-[#1c1c1c]">
                      {pricingBreakdown.currency} {Number(amountToPay).toLocaleString()}
                    </span>
                  </div>
                </>
              ) : (
                <p className="text-sm text-gray-500">No pricing breakdown available.</p>
              )}
            </div>
          </div>
        </div>
