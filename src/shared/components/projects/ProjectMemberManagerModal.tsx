...
const formatRole = (role: string | string[] | null | undefined): string | null => {
  if (!role) return null;
...
interface ProjectMemberManagerModalProps {
  isOpen: boolean;
  onClose: () => void;
  members?: Member[];
...
const ProjectMemberManagerModal: React.FC<ProjectMemberManagerModalProps> = ({
  isOpen,
  onClose,
  members = [],
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {member.isOwner && (
                          <span
                            className={`${statusPillClasses} border-sky-200 bg-sky-50 text-sky-700`}
                          >
                            <ShieldCheck size={12} />
                            Owner
                          </span>
                        ...
                        {member.isCurrent && !member.isOwner && (
                          <span
                            className={`${statusPillClasses} border-emerald-200 bg-emerald-50 text-emerald-700`}
                          >
                            Current
                          </span>
                        )}
                        {!member.isCurrent && (
                          <span
                            className={`${statusPillClasses} border-slate-200 bg-slate-50 text-slate-500`}
                          >
                            Suggested
                          </span>
                        )}
                      ...
                      {roleLabel && <p className="text-xs text-slate-400">Role: {roleLabel}</p>}
                    </div>
                  </label>
                );
              })
