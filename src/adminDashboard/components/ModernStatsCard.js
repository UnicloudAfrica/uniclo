...
const ModernStatsCard = ({
  ...
  const calculateChange = () => {
    ...
    const numericPrevious = parseFloat(
      previousValue.toString().replace(/[^0-9.-]/g, "")
    );
  ...
  const cardStyles = {
    ...
    transform:
      isHovered && onClick && !loading ? "translateY(-2px)" : "translateY(0)",
      ...
      {changeValue !== null && !loading && (
        <div
          style={{
            ...
            transition: prefersReducedMotion
              ? "none"
              : "all 0.4s cubic-bezier(0.4, 0, 0.2, 1) 0.1s",
          }}
