...
interface LeadCardProps {
  ...
  onCall?: (lead: LeadRecord) => void;
  onToggleFavorite?: (lead: LeadRecord) => void;
}

/**
 * LeadCard - Card view for individual lead
 */
const LeadCard = ({ lead, onView, onEmail, onCall, onToggleFavorite }: LeadCardProps) => {
  ...
  const getPriorityBadge = (score: number) => {
    ...
    if (score >= 60)
      return { label: "Medium", class: "bg-yellow-100 text-yellow-800 border-yellow-300" };
    return { label: "Low", class: "bg-gray-100 text-gray-600 border-gray-300" };
  };

  const formatStatusForDisplay = (status: string | null | undefined): string => {
    if (!status) {
      return "unknown";
    }
    ...
    <div className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:border-[rgb(var(--theme-color-300))] hover:shadow-lg">
      ...
      <div className="mb-4 flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${priority.class}`}
          >
            {priority.label}
          </span>
          <button
      ...
      <div className="mb-4 flex items-center justify-between">
        <span
          className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-semibold capitalize ${getStatusColorClass(lead.status)}`}
        >
