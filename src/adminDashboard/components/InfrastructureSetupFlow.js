...
const InfrastructureSetupFlow = ({ projectId, region }) => {
  ...
  const isComponentReady = (component) => {
  ...
  const getStatusIcon = (status, isReady) => {
    switch (status) {
      case "configured":
  ...
  const getStatusColor = (status, isReady) => {
    switch (status) {
      case "configured":
      ...
      {overallProgress >= 100 && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
          ...
          <p className="text-green-700">
            All infrastructure components have been configured successfully.
