 ...
 * either by their tenant's `force_client_2fa` policy or the platform's
 * `force_tenantless_client_2fa` flag (for direct platform clients).
 *
 * Routed to by `lib/api.ts` when the backend returns 403 with
 * `two_factor_enrollment_required: true` and `scope=client`.
 *
 * Calls the central /api/v1/2fa-setup → /api/v1/2fa-enable endpoints
 * since clientApi uses `baseURL = /api/v1/`. After enrolment the user
 * is shown one-time recovery codes before being released to the
 * client dashboard.
 */
const ClientTwoFactorEnroll = () => {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="text-sm text-slate-600 mt-2">
            Two-factor authentication is required for your account. Scan the QR code with an
            authenticator app (Google Authenticator, 1Password, Authy) and enter the 6-digit code
            below.
          </p>
        </div>
...
export default ClientTwoFactorEnroll;
