...
const AddEni = ({ isOpen, onClose, projectId, region: defaultRegion = "" }) => {
  ...
  const { data: networks, isFetching: isFetchingNetworks } =
    useFetchClientNetworks(projectId, form.region, {
          ...
          <div>
            ...
            >
              ...
              {networksList
                .filter(
                  (net) =>
                    (net.type || net?.meta?.network_type) === "vpc_network"
                )
                .map((v) => {
                  const providerId =
                    v.provider_resource_id ||
                    v.meta?.provider_resource_id ||
                    v.id;
            ...
            {!isFetchingNetworks &&
              ...
              form.region && (
                <div className="text-xs text-gray-500 mt-2 flex items-center justify-between">
                  <span>
                    No networks found for this region. Sync from provider?
                  </span>
                  <button
                    type="button"
                    onClick={handleRefreshNetworks}
