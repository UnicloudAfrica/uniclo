...
const ConfigurationStep = ({
  ...
  const encodedProjectIdentifierFromUrl = queryParams.get("id");
  const projectIdentifierFromUrl = encodedProjectIdentifierFromUrl
    ? decodeId(encodedProjectIdentifierFromUrl)
    : null;

  useEffect(() => {
    if (projectIdentifierFromUrl && projects && projects.length > 0) {
      const foundProject = projects.find(
        (project) => project.identifier === projectIdentifierFromUrl
      );

      if (foundProject && formData.selectedProject?.id !== foundProject.id) {
        handleSelectChange("selectedProject", foundProject.id, projects);
      }
    }
  }, [
  ...
  const isProjectSelectDisabled =
    isSubmissionPending ||
    (projectIdentifierFromUrl !== null && !isProjectsFetching);
