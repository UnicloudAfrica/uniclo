...
const createOnboardingSetting = async (payload: any) => {
...
const updateOnboardingSetting = async ({ id, payload }: any) => {
...
const deleteOnboardingSetting = async (id: any) => {
  await api("DELETE", `/onboarding-step-settings/${id}`);
  return id;
};

export const useFetchOnboardingSettings = (options: any = {}) => {
  return useQuery({
