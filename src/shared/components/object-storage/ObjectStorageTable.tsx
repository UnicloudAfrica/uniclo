...
const EmptyState = ({ icon: Icon = HardDrive, title, description, actions }: EmptyStateProps) => (
...
export interface Silo extends Record<string, unknown> {
  id?: string | number;
  name?: string;
  region?: string;
  url?: string;
...
export type PaginationState = Partial<{
  page: number;
  per_page: number;
}>;
...
interface ObjectStorageTableProps {
  ...
  onCreateSilo?: (accountId: string | number, data: { name: string }) => Promise<unknown> | void;
 ...
 */
const ObjectStorageTable: React.FC<ObjectStorageTableProps> = ({
  accounts = [],
  loading = false,
  error = null,
  onRetry,
  ...
  const resolveAccountKeys = (account: Account, index: number) => {
    const accountId = account.id ?? account.account_id ?? null;
    const fallbackKey = account.name ?? index;
    const accountKey = String(accountId ?? fallbackKey);
    const siloKey = accountId !== null ? String(accountId) : accountKey;
    return { accountId, accountKey, siloKey };
  };
  ...
  const paginationInfo = useMemo(() => {
    if (!paginationMeta) {
      return {
        ...
        perPage: resolveNumber(paginationState?.per_page, DEFAULT_PAGE_SIZES[0]),
    ...
    return {
      ...
      lastPage: resolveNumber(paginationMeta.last_page ?? paginationMeta.lastPage, 1),
    };
  }, [paginationMeta, paginationState, accounts.length]);

  const toggleRow = async (accountKey: string, accountId?: string | number | null) => {
    ...
    if (
      ...
      accountId !== null &&
      accountId !== undefined &&
      !silosByAccount?.[String(accountId)]
    ) {
      await onLoadSilos(accountId);
    }
  };

  const renderSilosSection = (
    account: Account,
    accountKey: string,
    accountId?: string | number | null
  ) => {
    const siloKey = accountId !== null && accountId !== undefined ? String(accountId) : accountKey;
    const silos = silosByAccount?.[siloKey] || [];
    const isLoading = siloLoading?.[siloKey];
    const siloError = siloErrors?.[siloKey];
    ...
    return (
      <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {onCreateSilo && accountId !== null && accountId !== undefined && (
            <button
              type="button"
              onClick={() => onCreateSilo(accountId, { name: "new-silo" })}
            ...
            silos.map((silo) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  {onDeleteSilo && accountId !== null && accountId !== undefined && (
                    <button
                      type="button"
                      onClick={() => onDeleteSilo(accountId, silo)}
    ...
    <table className="w-full text-left text-sm">
      ...
      <tbody>
        {filteredAccounts.map((account, index) => {
          const { accountId, accountKey } = resolveAccountKeys(account, index);
          return (
                ...
                <td className="px-6 py-4">
                  <button
                    ...
                    onClick={(event) => {
                      ...
                      toggleRow(accountKey, accountId);
                    }}
                    className="inline-flex items-center gap-1 text-xs font-semibold text-primary-600"
                  >
                    {expandedRows?.[accountKey] ? "Hide silos" : "View silos"}
                    <ArrowRight className="h-3.5 w-3.5" />
                  </button>
                </td>
              </tr>
              {expandedRows?.[accountKey] && (
                <tr className="border-b border-gray-100">
                  <td colSpan={6} className="px-6 py-4">
                    {renderSilosSection(account, accountKey, accountId)}
                  </td>
                </tr>
              )}
            </React.Fragment>
  ...
  const renderMobileAccountCard = (account: Account, index: number) => {
    const { accountId, accountKey } = resolveAccountKeys(account, index);
    const siloKey = accountId !== null && accountId !== undefined ? String(accountId) : accountKey;
    return (
      <div key={accountKey} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 text-xs text-gray-500">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-gray-700">
              {silosByAccount?.[siloKey]?.length || 0}
            </p>
          </div>
        </div>
        <button
        ...
        {enableSiloActions && (
          <button
            type="button"
            onClick={() => toggleRow(accountKey, accountId)}
        ...
        {expandedRows?.[accountKey] && (
          <div className="mt-4">{renderSilosSection(account, accountKey, accountId)}</div>
        )}
      </div>
    );
  };
    ...
    body = (
      <>
        ...
        <div className="space-y-4 p-4 md:hidden">
          {filteredAccounts.map((account, index) => renderMobileAccountCard(account, index))}
        </div>
      </>
    );
  }
