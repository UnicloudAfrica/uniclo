...
const categorizeFileType = (mimeType, fileName) => {
...
const DocumentViewerModal = ({ isOpen, onClose, document: documentData }) => {
  ...
  const { isPdf, isImage, isVideo, isAudio } = categorizeFileType(
    mimeType,
    documentData?.name
  );
  const canPreview =
    objectUrl && (isPdf || isImage || isVideo || isAudio) && !isError;
    ...
    return (
      <div className="absolute inset-0 flex flex-col items-center justify-center bg-white z-10 p-8 text-center">
        ...
        <p className="text-gray-500 text-sm">
          This file type cannot be previewed here. Use the download button to
          open it locally.
        </p>
      </div>
    );
