...
const BASE_STEPS_FAST_TRACK = [
  { id: "workflow", label: "Workflow & assignment", description: "Choose billing path and who owns this request." },
  { id: "services", label: "Service profiles", description: "Select regions, tiers, and contract length." },
  { id: "review", label: "Review & submit", description: "Validate totals and confirm provisioning." },
];

const BASE_STEPS_STANDARD = [
  { id: "workflow", label: "Workflow & assignment", description: "Choose billing path and who owns this request." },
  { id: "services", label: "Service profiles", description: "Select regions, tiers, and contract length." },
  { id: "payment", label: "Payment", description: "Generate payment options and share with finance." },
  { id: "review", label: "Review & submit", description: "Validate totals and confirm provisioning." },
];
...
const includesObjectStorageKeyword = (candidates = []) =>
  candidates.some((value) =>
    OBJECT_STORAGE_KEYWORDS.some((keyword) => value.includes(keyword))
  );

const isObjectStoragePricing = (pricing) => {
...
const AdminObjectStorageCreate = () => {
  ...
  } = useFetchProductPricing(primaryRegion, "object_storage_configuration", {
  ...
  const determineTierCurrency = (tier) => {
  ...
  const summaryCurrency =
    selectedCurrency ||
    resolvedProfiles.find((profile) => profile.currency)?.currency ||
    "USD";
  ...
  const validateWorkflowStep = () => {
    ...
    setErrors((prev) => {
      const next = { ...prev };
      ["countryCode", "assignment"].forEach((key) => {
  ...
  const handleSubmit = (event) => submitOrder(event, false);
  const handleFastTrackSubmit = (event) => submitOrder(event, true);
  const handleGeneratePaymentOptions = (event) =>
    submitOrder(event, false, { skipReset: true });

  const handlePaystackCard = (option) => {
  ...
  const isNextDisabled = useMemo(
    () => {
      ...
      const isBeforeReviewWithoutPayment = !isFastTrack && steps[activeStep + 1]?.id === "review" && !isPaymentComplete;
                ...
                <ModernCard variant="outlined" padding="lg" className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <ModernSelect
                      ...
                      helper={
                        isCountriesLoading
                          ? "Loading billing countries..."
                          : billingCountryHelper
                      }
                  ...
                  {isPaymentComplete && (
                    <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
                      ...
                      <p className="text-xs text-emerald-700">
                        This transaction is confirmed. You can continue to review without making another payment.
                      </p>
                    </div>
                  )}
                  <div className="flex flex-wrap items-center justify-end gap-3 pt-2">
                ...
                <ModernCard variant="outlined" padding="lg" className="space-y-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        Confirm ownership, billing country, and tier choices before triggering the workflow.
                      </p>
                    </div>
                    <StatusPill
                  ...
                  <dl className="grid gap-3 text-sm sm:grid-cols-2">
                    ...
                    <div className="rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
                      ...
                      <dd className="font-semibold text-slate-900">
                        {isFastTrack ? "Fast-track provisioning" : "Payment required before provisioning"}
                      </dd>
                    </div>
                    <div className="rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
                      <dt className="text-xs uppercase tracking-wide text-slate-500">
                  ...
                  <p className="text-xs text-slate-500">
                    Need to adjust details? Jump back to earlier steps with the stepper above, then return here to submit.
                  </p>
                  {!isFastTrack && (
                    <div className="space-y-2">
            ...
            <div className="space-y-6 w-full max-w-xl md:max-w-none md:w-full justify-self-end">
              <ModernCard variant="elevated" padding="lg" className="space-y-4 lg:sticky lg:top-4">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  {(!backendPricingLines || backendPricingLines.length === 0) && resolvedProfiles.length === 0 ? (
                    <p className="text-xs text-slate-500">
                    ...
                    <div className="space-y-2">
                      {(backendPricingLines || resolvedProfiles).map((entry, index) => {
                        const isBackend = Array.isArray(backendPricingLines) && backendPricingLines.length > 0;
                        const line = isBackend ? entry : resolvedProfiles[index];
                        if (!line) return null;
                        const lineName = isBackend
                          ? `${line.region || ""} ${line.name || ""}`.trim() || "Object storage tier"
                          : line.region || "N/A";
                ...
                <dl className="space-y-3 text-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <dd className="font-medium text-slate-900">
                      {formatCurrency(
                        displayedTotals.subtotal,
                        displayedTotals.currency
                      )}
                    </dd>
                  </div>
                  <div className="flex items-center justify-between">
