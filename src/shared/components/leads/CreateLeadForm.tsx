...
const leadStageStatusOptions = ["pending", "in_progress", "completed", "skipped"] as const;
...
type LeadFormData = Omit<
  Lead,
  "id" | "created_at" | "updated_at" | "lead_stage" | "assigned_to"
> & {
...
type LeadFormErrors = Partial<Record<LeadFormErrorKey, string>>;

export type CreateLeadFormProps = {
  isOpen?: boolean;
  onClose?: () => void;
  mode?: "modal" | "page";
  leadTypesData?: unknown;
  leadTypesLoading?: boolean;
  countries?: CountryOption[];
  countriesLoading?: boolean;
  countriesError?: boolean;
  assignees?: LeadAssigneeOption[];
  isAssigneesLoading?: boolean;
  onSubmit: (payload: LeadCreatePayload) => void;
  isSubmitting?: boolean;
  pageMaxWidthClass?: string;
  modalMaxWidthClass?: string;
  assigneePlaceholder?: string;
  assigneeLoadingPlaceholder?: string;
  stageAssigneePlaceholder?: string;
  stageAssigneeLoadingPlaceholder?: string;
};
...
const CreateLeadForm = ({
  ...
  const leadTypeOptions = useMemo<LeadTypeOption[]>(
  ...
  const normalizeLeadTypeValue = useCallback(
    (value: string) => ensureLeadTypeValue(value, leadTypeOptions),
    [leadTypeOptions]
  );
  ...
  const updateLeadStageFormData = (field: keyof LeadStageData, value: string) => {
    setFormData((prev) => ({
      ...prev,
      lead_stage: { ...prev.lead_stage, [field]: value },
    }));
    setErrors((prev) => ({ ...prev, [`stage_${field}`]: undefined }));
  };
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    const leadData: LeadCreatePayload = { ...formData };
    if (!formData.lead_stage.stage_name) {
      delete leadData.lead_stage;
    }
  ...
  return (
    <FormLayout
      ...
      aside={asideContent}
      footer={footer}
      maxWidthClass={isPageMode ? pageMaxWidthClass : modalMaxWidthClass}
    >
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              >
                ...
                {leadTypeOptions.map((option: LeadTypeOption) => (
            ...
            <div>
              ...
              >
                <option value="">
                  {isAssigneesLoading ? assigneeLoadingPlaceholder : assigneePlaceholder}
                </option>
                {!isAssigneesLoading &&
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              >
                <option value="">
                  {isAssigneesLoading ? stageAssigneeLoadingPlaceholder : stageAssigneePlaceholder}
                </option>
                {!isAssigneesLoading &&
                  assignees.map((assignee) => (
                    <option key={assignee.value} value={assignee.value}>
