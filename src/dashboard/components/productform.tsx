    ...
    {steps.map((step: any, index: any) => (
      <React.Fragment key={step}>
        <div className="flex flex-col items-center text-center">
          ...
          <p
            className={`text-xs mt-2 text-center ${
              index <= currentStep ? "text-[var(--theme-color)]" : "text-gray-500"
            }`}
          >
            {step}
          </p>
        ...
        {index < steps.length - 1 && (
          <div
            className={`flex-1 h-0.5 mx-4 ${index < currentStep ? "bg-[var(--theme-color)]" : "bg-gray-200"}`}
          />
        )}
      </React.Fragment>
    ))}
  ...
  const validateStep = () => {
    ...
    if (currentStep === 0) {
      ...
      if (!formData.networking_requirements)
        newErrors.networking_requirements = "Networking requirements are required";
      if (!formData.accomplish_goal) newErrors.accomplish_goal = "Goal is required";
    } else if (
      (currentStep === 2 && formData.solution === "Compute") ||
      ...
      <div className="space-y-4">
        ...
        {currentStep === 1 && formData.solution !== "Compute" && (
          <>
            <div>
              ...
              {errors.data_type && <p className="text-red-500 text-xs mt-1">{errors.data_type}</p>}
            </div>
            <div>
              <label
              ...
              <input
                ...
                onChange={(e) => updateFormData("usable_capacity", e.target.value)}
              ...
              {errors.usable_capacity && (
                <p className="text-red-500 text-xs mt-1">{errors.usable_capacity}</p>
              )}
            </div>
            <div>
              <label htmlFor="term_of_use" className="block text-sm font-medium text-gray-700 mb-2">
                Term of Use *
              </label>
              <span
                className={`w-full input-field block transition-all ${
              ...
              >
                <select
                  ...
                  onChange={(e) => updateFormData("term_of_use", e.target.value)}
                  className="w-full bg-transparent outline-none"
                >
                  <option value="" disabled>
                    Select term
              ...
              {errors.term_of_use && (
                <p className="text-red-500 text-xs mt-1">{errors.term_of_use}</p>
              )}
            </div>
          </>
        )}
          ...
          <>
            <div>
              ...
              {errors.application_use && (
                <p className="text-red-500 text-xs mt-1">{errors.application_use}</p>
              )}
            </div>
            <div>
              <label htmlFor="vm_count" className="block text-sm font-medium text-gray-700 mb-2">
                How many VMs? *
              </label>
              <span
                className={`w-full input-field block transition-all ${
              ...
              {errors.vm_count && <p className="text-red-500 text-xs mt-1">{errors.vm_count}</p>}
            </div>
            <div>
              <label
              ...
              >
                <select
                  ...
                  onChange={(e) => updateFormData("networking_requirements", e.target.value)}
                  className="w-full bg-transparent outline-none"
                >
                  <option value="" disabled>
              ...
              {errors.networking_requirements && (
                <p className="text-red-500 text-xs mt-1">{errors.networking_requirements}</p>
              )}
            </div>
            <div>
              <label
              ...
              <input
                ...
                onChange={(e) => updateFormData("accomplish_goal", e.target.value)}
              ...
              {errors.accomplish_goal && (
                <p className="text-red-500 text-xs mt-1">{errors.accomplish_goal}</p>
              )}
            </div>
          </>
        )}
        ...
        <div className="flex gap-4 mt-8">
          ...
          <button
            onClick={handleNext}
            disabled={isPending}
            className="flex-1 bg-[var(--theme-color)] hover:bg-[rgb(var(--theme-color-400))] text-white font-semibold py-3 px-4 rounded-lg transition-colors focus:outline-none focus:ring-1 focus:ring-[var(--theme-color)] focus:ring-offset-2 flex items-center justify-center"
          >
            {(currentStep === 2 && formData.solution === "Compute") ||
            (currentStep === 3 && formData.solution !== "Compute")
              ? "Complete"
