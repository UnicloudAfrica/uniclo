...
import { useDownloadAdminTransactionReceipt, useFetchAdminTransaction } from "../../hooks/adminHooks/paymentHooks";
  ...
  const metrics = useMemo(() => {
    return [
      ...
      },
      {
        ...
        description: payment.reference ? `Reference ${payment.reference}` : "Awaiting reference sync",
        ...
        <table className="min-w-full divide-y divide-slate-200">
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            {lineItems.map((item, index) => (
              <tr key={`${item.identifier || item.description || index}-${index}`}>
                <td className="px-5 py-4 text-sm text-slate-700">
                  ...
                  {item.itemable?.identifier && (
                    <span className="text-xs text-slate-500">
                      Ref • {item.itemable.identifier}
                    </span>
                  )}
                </td>
                <td className="px-5 py-4 text-sm text-slate-600">
                  {(item.frequency || "Recurring").replace(/_/g, " ")}
                </td>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <h3 className="text-lg font-semibold text-slate-900">
              Instances tied to this payment
            </h3>
            <p className="text-sm text-slate-500">
              Review which workloads were provisioned as part of this
              transaction.
            </p>
          </div>
        </div>
          ...
          <table className="min-w-full divide-y divide-slate-200">
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {data.instances.map((instance) => (
                <tr key={instance.identifier || instance.id}>
                  <td className="px-5 py-4 text-sm text-slate-700">
                    <p className="font-semibold text-slate-900">
                  ...
                  <td className="px-5 py-4 text-sm text-slate-600 uppercase">
                    {instance.region || "—"}
                  </td>
                  <td className="px-5 py-4 text-sm text-slate-600">
                    {instance.provider || "—"}
        ...
        {isError ? (
          <ModernCard className="flex flex-col items-center justify-center gap-4 border border-rose-200 bg-rose-50/80 py-12 text-center text-rose-700">
            ...
            <p className="text-sm max-w-lg text-rose-600">
              We couldn't retrieve this payment record. It may have been removed
              or there may be a temporary service disruption.
            </p>
          ...
          <>
            ...
            <ModernCard className="space-y-6 border border-slate-200/80 bg-white/95 shadow-sm backdrop-blur">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-slate-500">
                    Quickly review who paid, how it was processed, and any
                    gateway references tied to the charge.
                  </p>
                </div>
            ...
            <ModernCard className="space-y-6 border border-slate-200/80 bg-white/95 shadow-sm">
              <div className="space-y-1">
                ...
                <h2 className="text-lg font-semibold text-slate-900">
                  Billable services included
                </h2>
                <p className="text-sm text-slate-500">
                  Each line item represents an infrastructure component or add-on
                  that was billed as part of this transaction.
                </p>
              </div>
              {renderLineItems()}
