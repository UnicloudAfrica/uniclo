...
import {
  ...
  STATUS_OPTIONS,
  STATUS_TONES,
} from "../../adminDashboard/pages/adminOnboardingReview";
...
const SubjectList = ({ label, items, selectedKey, onSelect }) => (
    ...
    {items.length === 0 ? (
      ...
      <div className="mt-3 space-y-2">
        {items.map((item) => {
          ...
          const currentStep = item.steps.find((step) => step.id === item.current_step) ?? item.steps[0];
...
const TenantOnboardingOverview = () => {
  ...
  useEffect(() => {
    ...
    if (selectedSubject.source === "queue") {
      ...
      if (lookup) {
        ...
        setActiveStep(
          lookup.current_step ?? lookup.steps[0]?.id ?? null
        );
      }
      return;
    }
  ...
  const renderPayload = () => {
    if (!submissionDetail) {
      return (
        <p className="text-sm text-gray-500">No submission for this step yet.</p>
      );
    }
    ...
    if (entries.length === 0) {
      return (
        <p className="text-sm text-gray-500">No data provided yet.</p>
      );
    }
          ...
          <tbody>
            {entries.map(([key, value]) => (
              <tr key={key} className="border-b border-gray-100">
                <td className="w-1/3 px-4 py-3 font-medium text-gray-700">
                  {key}
                </td>
  ...
  return (
    <>
      ...
      <main
        ref={contentRef}
        className="dashboard-content-shell overflow-y-auto p-6 md:p-8"
      >
        <div className="flex items-center justify-between">
          <div>
                            ...
                            <div className="space-y-2">
                              ...
                              {!canDecide && (
                                <p className="text-xs text-amber-600">
                                  This step has not been submitted yet. Ask the submitter to provide the required details before approving.
                                </p>
                              )}
                            </div>
                            <button
