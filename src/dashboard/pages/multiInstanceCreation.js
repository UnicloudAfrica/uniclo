...
const InstanceConfigCard = ({
  ...
  const { data: securityGroups } = useFetchTenantSecurityGroups(
    projectIdentifier,
    selectedRegion,
    { enabled: !!projectIdentifier && !!selectedRegion }
  );
  const { data: keyPairs } = useFetchTenantKeyPairs(
    projectIdentifier,
    selectedRegion,
    { enabled: !!projectIdentifier && !!selectedRegion }
  );
  const { data: subnets } = useFetchTenantSubnets(
    projectIdentifier,
    selectedRegion,
    { enabled: !!projectIdentifier && !!selectedRegion }
  );
  const { data: networksResponse } = useFetchTenantNetworks(
    projectIdentifier,
    selectedRegion,
    { enabled: !!projectIdentifier && !!selectedRegion }
  );
...
export default function MultiInstanceCreation() {
  ...
  // Fetch projects filtered by region (used as baseline; cards fetch per-region too)
  ...
  const steps = ["Overview", "Configurations", "Review & Launch", "Payment"];
  const stepDescriptions = [
    "Review how the multi-instance launch process works before configuring resources.",
  ...
  const renderOverviewCard = () => (
    <ModernCard padding="lg" className="space-y-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Confirm the plan for this deployment before adding instance configurations.
          </p>
        </div>
        <StatusPill label="Step 1 of 4" tone="info" />
      </div>
      <ul className="space-y-3 text-sm text-slate-600">
        <li className="flex items-start gap-3">
          <CheckCircle className="mt-1 h-4 w-4 text-primary-500" />
          <div>
            <p className="font-medium text-slate-900">Define workload goals</p>
            <p className="text-xs text-slate-500">
              Outline how many environments you need, the target regions, and any networking constraints.
            </p>
          </div>
        </li>
        <li className="flex items-start gap-3">
          <CheckCircle className="mt-1 h-4 w-4 text-primary-500" />
          <div>
            <p className="font-medium text-slate-900">Gather project identifiers</p>
            <p className="text-xs text-slate-500">
              Each configuration can be linked to an existing project so billing and resources stay organised.
            </p>
          </div>
        </li>
        <li className="flex items-start gap-3">
          <CheckCircle className="mt-1 h-4 w-4 text-primary-500" />
          <div>
            <p className="font-medium text-slate-900">Set provisioning expectations</p>
            <p className="text-xs text-slate-500">
              Fast track begins provisioning immediately after payment; otherwise requests queue for manual release.
            </p>
          </div>
        </li>
      </ul>
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs text-slate-500">
        <p>
          You are launching from your tenant workspace—orders, pricing, and notifications automatically stay within this account.
        </p>
      </div>
    </ModernCard>
  );
          ...
          <ModernCard padding="lg" className="space-y-3">
            ...
            <ul className="space-y-2 text-xs text-slate-500">
              <li className="flex items-start gap-2">
                ...
                <span>Step 1: Review launch prerequisites so you know the projects, regions, and timelines involved.</span>
              </li>
              <li className="flex items-start gap-2">
                <CheckCircle className="h-4 w-4 text-primary-500" />
