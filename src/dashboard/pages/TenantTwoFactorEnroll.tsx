 ...
 * into 2FA by their tenant's `force_2fa` policy.
 *
 * Routed to by `lib/api.ts` when the backend returns 403 with
 * `two_factor_enrollment_required: true` and scope=tenant.
 *
 * Calls the same /2fa-setup → /2fa-enable endpoints as the admin
 * variant — they're shared across all roles.
 */
const TenantTwoFactorEnroll = () => {
  ...
  useEffect(() => {
    const loadSetup = async () => {
      setLoading(true);
      try {
        const response = (await tenantApi("GET", "/2fa-setup")) as Record<string, unknown>;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const response = (await tenantApi("POST", "/2fa-enable", { otp })) as {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="text-sm text-slate-600 mt-2">
            Your tenant administrator requires 2FA for all users. Scan the QR code with an
...
export default TenantTwoFactorEnroll;
