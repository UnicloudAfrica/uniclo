...
const tenantBusinessSteps: OnboardingStepDefinition[] = [
  {
    ...
    description: "Tell us about your business so we can tailor the experience to your operations.",
    ...
    fields: [
      {
        ...
        fields: [
          ...
          },
          {
            ...
            type: "file",
            helperText: "Passport, driver's licence, or any document matching the ID type.",
            required: true,
          },
        ],
      },
  ...
  },
  {
    ...
    description: "Upload your brand assets so the CRM reflects your identity from day one.",
...
const maybeClientBusinessSteps = [
  ...["business_profile", "compliance_documents", "founders_directors"].map((stepId) => {
    ...
    return template
      ? { ...template, fields: template.fields ? [...template.fields] : template.fields }
      : null;
  }),
];

const clientBusinessSteps: OnboardingStepDefinition[] = maybeClientBusinessSteps.filter(
  (step) => step !== null
) as OnboardingStepDefinition[];

const internalClientBusinessSteps: OnboardingStepDefinition[] = clientBusinessSteps.map((step) => ({
  ...step,
  fields: step.fields ? [...step.fields] : step.fields,
}));

const tenantClientIndividualSteps: OnboardingStepDefinition[] = [
  {
    id: "client_profile",
    label: "Client Profile",
    description: "Share your personal details so we can verify and activate your services.",
    fields: [
      {
        id: "first_name",
        label: "First name",
...
export const STEP_CONFIG: Record<string, OnboardingStepDefinition[]> = {
...
export const getStepsForTarget = (target: string): OnboardingStepDefinition[] =>
