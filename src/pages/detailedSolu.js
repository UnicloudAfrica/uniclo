...
const DetailedSolution = ({ }) => {
    ...
    useEffect(() => {
       ...
       .then((querySnapshot) => {
           ...
           const soluData = { id: doc.id, ...doc.data() };
           if (id !== doc.id) {
               otherSoluData.push(soluData);
           }
           });
           setOtherSolutions(otherSoluData);
       })
       .catch((error) => {
    ...
    return ( 
        ...
        <div className="mt-[8em] px-4 md:px-8 lg:px-16 w-full font-Outfit text-[#121212]">
            <p className=" font-medium text-3xl md:text-4xl text-center">{selectedSolutionItem.topic}</p>
            <p className=" text-center font-normal mt-3 text-[#676767] text-lg md:text-xl ">{selectedSolutionItem.desc}</p>
            <div className=" w-full h-[350px] md:h-[500px] my-16 bg-[#F5F5F4] rounded-[20px]" style={{ backgroundImage: `url(${selectedSolutionItem.url})`, backgroundSize: 'cover' }}>

            </div>
            <p className=" font-medium text-[30px] leading-[40px] text-center"></p>
            <p style={{ whiteSpace: 'pre-line' }} className=" mt-3 text-sm whitespace-pre-line text-justify text-[#676767] md:px-[15%] font-normal" dangerouslySetInnerHTML={{ __html: selectedSolutionItem.content }}/>
            ...
            <p className=" font-medium text-xl md:text-3xl text-center mt-16">Explore other solutions</p>
