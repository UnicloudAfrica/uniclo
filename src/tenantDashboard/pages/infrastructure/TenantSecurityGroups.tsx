...
const TenantSecurityGroups: React.FC = () => {
  ...
  const handleViewRules = (sg: SecurityGroup) => {
    navigate(
      `/dashboard/infrastructure/security-group-rules?project=${projectId}&sg=${sg.id}&name=${sg.name || "Security Group"}`
    );
  };
            ...
            <tbody className="divide-y divide-gray-100">
              {securityGroups.map((sg: SecurityGroup) => (
                <tr key={sg.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 font-mono text-xs text-gray-500">{sg.vpc_id}</td>
                  <td className="py-3 px-4">
                  ...
                  <td className="py-3 px-4 text-right">
                    <ModernButton variant="secondary" size="sm" onClick={() => handleViewRules(sg)}>
