/**
 * Tenant availability-zone loading in the invoice wizard's object-storage step.
 *
 * Regression guard for the bug where StorageItemBuilder imported the
 ...
 * 404, leaving the AZ dropdown empty so a tenant operator could not add an
 * object-storage line item.
 *
 * Here the AZ fetch is driven by `formData.object_storage_region` (not the
 * compute `region` field). The real tenant route is
 * `/tenant/v1/admin/regions/{code}/availability-zones` (the `admin` prefix
 * group in routes/tenant.php). The context-aware shared hook resolves that
 * via apiRegistry.tenant.urlPrefix === "/admin". This fetch mock therefore
 * serves AZs ONLY at the correctly-prefixed URL and 404s everything else,
...
const baseFormData: InvoiceFormData = {
  ...
  floating_ip_id: null,
  floating_ip_count: 0,
  // The object-storage region drives StorageItemBuilder's AZ fetch.
  object_storage_region: "uni-ng",
...
const baseProps = {
  ...
  isObjectStorageProductsFetching: false,
  onAddObjectStorageRequest: vi.fn(),
};
...
describe("StorageItemBuilder — tenant availability zones", () => {
  beforeEach(() => {
    fetchMock.mockClear();
  ...
  it("loads availability zones for a tenant via the /admin-prefixed route", async () => {
    renderInTenantContext(<StorageItemBuilder {...baseProps} />);

    // The dropdown populates — proving a tenant can pick an AZ (and therefore
    // go on to add an object-storage line item).
    expect(
