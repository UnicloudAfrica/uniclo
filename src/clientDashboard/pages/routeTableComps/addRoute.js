...
import { useFetchClientNetworkInterfaces } from "../../../hooks/clientHooks/networkHooks";
...
const AddRoute = ({
  ...
  const { mutate: createRoute, isPending } = useCreateClientRoute();
  const { data: igws } = useFetchClientIgws(projectId, form.region, {
    enabled: !!projectId && !!form.region,
  });
  const { data: enis } = useFetchClientNetworkInterfaces(projectId, form.region, {
    enabled: !!projectId && !!form.region,
  });

  useEffect(() => {
    if (defaultRegion && !form.region) {
      setForm((p) => ({ ...p, region: defaultRegion }));
    }
  }, [defaultRegion, form.region]);

  useEffect(() => {
    if (routeTableId && !form.route_table_id) {
      setForm((p) => ({ ...p, route_table_id: routeTableId }));
    }
  ...
  }, [defaultGatewayId, form.target_id, form.target_type]);
  ...
  const validate = () => {
    ...
    if (!form.destination_cidr_block)
      e.destination_cidr_block = "Destination CIDR is required";
  ...
  const eniList = Array.isArray(enis) ? enis : [];
  const tables = Array.isArray(routeTables) ? routeTables : [];
        ...
        <form onSubmit={submit} className="p-4 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, route_table_id: e.target.value }))
              }
            ...
            >
              ...
              {tables.map((rt) => (
                <option
                  key={rt.id || rt.route_table?.id}
                  value={rt.id || rt.route_table?.id}
                >
            ...
            <div>
              ...
              {form.target_type === "gateway_id" ? (
                ...
                <input
                  ...
                  placeholder={
                    form.target_type === "instance_id" ? "i-..." : "nat-..."
                  }
                />
              )}
              {errors.target_id && (
