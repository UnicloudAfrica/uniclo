...
import { ModernCard, ModernButton, ModernTable } from "../../ui";
...
const TotalsCard = ({ amounts }) => (
  <div className="w-full space-y-2 rounded-3xl border border-slate-200 bg-gradient-to-br from-primary-50 to-white p-6">
    ...
    <div className="flex items-center justify-between border-t border-slate-200 pt-2 text-sm text-slate-600">
      <span>Subtotal</span>
      <span className="font-semibold">{formatCurrency(amounts.subtotal, amounts.currency)}</span>
    </div>
    <div className="flex items-center justify-between text-sm text-slate-600">
      <span>Tax</span>
      <span className="font-semibold">{formatCurrency(amounts.tax, amounts.currency)}</span>
    </div>
    <div className="flex items-center justify-between border-t-2 border-slate-300 pt-3 text-lg font-bold text-slate-900">
      <span>Total</span>
      <span className="text-primary-600">{formatCurrency(amounts.total, amounts.currency)}</span>
    </div>
  </div>
);
          ...
          const columns = [
            {
              ...
              header: "DESCRIPTION",
              render: (_, item) => (
                <span className="text-slate-900">{item.name || item.description || "N/A"}</span>
              ),
            },
            {
              key: "quantity",
              header: "QUANTITY",
              align: "right",
              render: (_, item) => <span className="text-slate-600">{item.quantity || "—"}</span>,
            },
            {
              key: "unit_price",
              header: "UNIT PRICE",
          ...
          return (
            <ModernCard key={invoice.invoice_number} padding="xl" className="space-y-6">
              ...
              {invoice.line_items?.length > 0 && (
                <div>
                  <h5 className="mb-3 text-sm font-semibold text-slate-900">Line Items</h5>
