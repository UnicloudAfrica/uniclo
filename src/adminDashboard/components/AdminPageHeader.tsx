...
const AdminPageHeader: React.FC<AdminPageHeaderProps> = ({
  ...
  const renderBreadcrumbLabel = (item: BreadcrumbItem, isLast: boolean) => {
    const baseClasses = "font-medium transition-colors";
    if (item?.href && !isLast) {
      return (
        <Link to={item.href} className={`${baseClasses} text-[#288DD1] hover:text-[#0F75B5]`}>
          {item.label}
        </Link>
      );
    }
    ...
    <div className="bg-white border-b border-gray-200 px-6 md:px-8 py-6 space-y-4">
            ...
            {items.map((item, index) => {
              ...
              return (
                <React.Fragment key={`${item?.label}-${index}`}>
                  {index > 0 && (
                    <ChevronRight className="w-4 h-4" color={`${BREADCRUMB_COLOR}66`} />
                  )}
      ...
      {(title || description || actions || subHeaderContent) && (
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {actions && (
            <div className="flex flex-col sm:flex-row gap-3 sm:items-center">{actions}</div>
          )}
        </div>
      )}
    </div>
