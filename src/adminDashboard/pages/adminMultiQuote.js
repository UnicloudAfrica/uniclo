...
const AdminMultiQuote = () => {
  ...
  const validateStep = (step = currentStep) => {
    ...
    if (step === 0) {
      ...
      if (pricingRequests.length === 0 && objectStorageRequests.length === 0) {
        newErrors.general = "Please add at least one item (compute or object storage) to the quote.";
      }
  ...
  const headerMeta = (
    <div className="flex flex-wrap items-center gap-3 text-xs sm:text-sm text-gray-500">
      ...
      <span>
        {pricingRequests.length + objectStorageRequests.length} item{pricingRequests.length + objectStorageRequests.length === 1 ? "" : "s"}
      </span>
  ...
  const uniqueRegions = Array.from(
    new Set([
      ...pricingRequests.map((item) => item.region),
      ...objectStorageRequests.map((item) => item.region),
    ].filter(Boolean))
  );

  const itemsMetadata = [
    { label: "Total Items", value: (pricingRequests.length + objectStorageRequests.length).toString() || "0" },
