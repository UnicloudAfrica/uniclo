...
import PricingCalculatorConfig from "../../pages/pricingCalculator/PricingCalculatorConfig";
import CalculatorSummaryStep from "../../pages/calculatorComps/calculatorSummaryStep";
...
const SharedPricingCalculator = ({ mode = "admin", onExit }) => {
    ...
    const { mutateAsync: calculatePricing, isPending: isCalculatingMutation } = useSharedCalculatorPricing();
    ...
    const validateConfig = () => {
        ...
        if (calculatorData.pricing_requests.length === 0 && calculatorData.object_storage_items.length === 0) {
        ...
        try {
            const payload = {
                pricing_requests: calculatorData.pricing_requests.map(req => ({
                    ...
                    volume_types: req.volumes?.map(vol => ({
    ...
    return (
        <div className="pb-24 md:pb-12">
            ...
            <div className="mb-6 flex items-center gap-2">
                <div className={`flex items-center gap-2 rounded-full px-4 py-1.5 text-sm font-medium ${activeStep === 1 ? 'bg-primary-50 text-primary-700 ring-1 ring-primary-200' : 'text-slate-500'}`}>
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-white text-xs font-bold shadow-sm">1</span>
                    Configuration
                </div>
                <div className="h-px w-8 bg-slate-200"></div>
                <div className={`flex items-center gap-2 rounded-full px-4 py-1.5 text-sm font-medium ${activeStep === 2 ? 'bg-primary-50 text-primary-700 ring-1 ring-primary-200' : 'text-slate-500'}`}>
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-white text-xs font-bold shadow-sm border border-slate-200">2</span>
                    Summary
                </div>
            </div>
