...
const EBSVolumes = ({ selectedRegion, onMetricsChange }) => {
  ...
  const avgReadIOPS = useMemo(() => {
    ...
    return (
      rows.reduce((acc, volume) => acc + Number(volume.read_iops_limit || 0), 0) /
      rows.length
    );
  }, [rows]);
  ...
  const avgWriteIOPS = useMemo(() => {
    if (!rows.length) return 0;
    return (
      rows.reduce((acc, volume) => acc + Number(volume.write_iops_limit || 0), 0) /
      rows.length
    );
  }, [rows]);
