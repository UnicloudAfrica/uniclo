...
const PaymentStep = ({
  ...
  const pricing_breakdown = instanceRequestResponse?.metadata?.pricing_breakdown;
    ...
    <div className="text-center space-y-6 py-10 w-full">
      ...
      {isSubmissionPending ? (
        ...
        <div className="text-red-500">
          ...
          <p>{generalError || "Failed to process your order. Please try again."}</p>
        </div>
      ) : instanceRequestResponse ? (
        <div className="space-y-6">
          <div>
            ...
            <span className="w-full px-2 py-4 border border-[#E9EAF4] rounded-[10px] block">
              ...
              instanceRequestResponse.payment_gateway_options.length > 0 ? (
                ...
                >
                  {instanceRequestResponse.payment_gateway_options.map((option) => (
          ...
          {selectedPaymentOption?.payment_type.toLowerCase() === "bank transfer" &&
            selectedPaymentOption?.details && (
              <div className="bg-[#F8F8F8] rounded-lg py-4 px-6 text-left">
                <h3 className="text-sm font-medium text-[#1C1C1C] mb-4">Bank Account Details</h3>
                <div className="space-y-2">
                  <div className="flex w-full items-center justify-between">
                    <span className="text-sm font-normal text-[#676767]">Account formatPrice:</span>
                    <span className="text-sm font-normal text-[#1c1c1c]">
                      {selectedPaymentOption.details.account_formatPrice || "N/A"}
                    </span>
                  </div>
                  <div className="flex w-full items-center justify-between">
                    <span className="text-sm font-normal text-[#676767]">Account Name:</span>
                    <span className="text-sm font-normal text-[#1c1c1c]">
                      {selectedPaymentOption.details.account_name || "N/A"}
                    </span>
                  </div>
                  <div className="flex w-full items-center justify-between">
                    <span className="text-sm font-normal text-[#676767]">Provider:</span>
                    <span className="text-sm font-normal text-[#1c1c1c]">
                      {selectedPaymentOption.details.provider || "N/A"}
                    </span>
                  </div>
                  <div className="flex w-full items-center justify-between">
                    <span className="text-sm font-normal text-[#676767]">Currency:</span>
                    <span className="text-sm font-normal text-[#1c1c1c]">
          ...
          <div className="bg-[#F8F8F8] rounded-lg py-4 px-6 text-left">
            ...
            {pricing_breakdown ? (
              <>
                {pricing_breakdown.lines.map((line: any) => (
                  <div key={line.slug} className="flex w-full items-center justify-between mb-2">
                    <span className="text-sm font-normal text-[#676767]">{line.name}:</span>
                    <span className="text-sm font-normal text-[#1c1c1c]">
                ...
                <div className="flex w-full items-center justify-between mb-2">
                  <span className="text-sm font-medium text-[#676767]">
                    Tax ({formatPrice((pricing_breakdown.tax / pricing_breakdown.subtotal) * 100)}%
                    VAT):
                  </span>
                  <span className="text-sm font-normal text-[#1c1c1c]">
                    {getCurrencySymbol(pricing_breakdown.currency)}{" "}
        ...
        <p className="text-gray-700">Proceed to previous step to generate payment details.</p>
      )}
    </div>
  );
};
