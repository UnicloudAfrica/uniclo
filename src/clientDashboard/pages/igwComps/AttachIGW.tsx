...
const AttachIgwModal: React.FC<AttachIgwModalProps> = ({
  isOpen,
  onClose,
  projectId,
  region = "",
  ...
  const { data: vpcRaw, isFetching } = useFetchClientVpcs(projectId, region, {
    enabled: isOpen && !!projectId && !!region,
  });
  const vpcs = useMemo(() => {
    if (Array.isArray((vpcRaw as any)?.data)) return (vpcRaw as any).data;
    if (Array.isArray(vpcRaw)) return vpcRaw;
    return [];
  }, [vpcRaw]);

  const { mutate: attachIgw, isPending: isAttaching } = useAttachClientIgw();
  const { mutate: detachIgw, isPending: isDetaching } = useDetachClientIgw();
