...
const deriveProgressBuckets = (steps, statuses) => {
  return steps.reduce(
    (acc, step) => {
...
const OnboardingStatusBoard = ({
    ...
    const fetchStatuses = async () => {
      ...
      try {
        const stepResults = await Promise.all(
          steps.map(async (step) => {
            try {
              ...
              console.error(
                `Failed to load onboarding submission for ${step.id}`,
                stepError
              );
              return [
                step.id,
        ...
        if (!isCancelled) {
          ...
          setError(
            fetchError?.message ??
              "Unable to load onboarding submissions right now."
          );
          setStatuses({});
        }
      } finally {
  ...
  const progressBuckets = useMemo(
    () => deriveProgressBuckets(steps, statuses),
    [steps, statuses]
  );
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <OnboardingSummaryCard
          label="Approved"
          value={progressBuckets.completed}
          tone="success"
        />
        <OnboardingSummaryCard
          label="In Review"
          value={progressBuckets.inProgress}
          tone="info"
        />
        <OnboardingSummaryCard
          label="Pending"
          value={progressBuckets.pending}
          tone="neutral"
        />
        <OnboardingSummaryCard
          label="Escalated"
      ...
      {!hasValidSubject ? (
        ...
        <div className="space-y-4">
          {steps.map((step) => {
            ...
            const statusLabel =
              STATUS_LABELS[snapshot.status] ?? snapshot.status ?? "Unknown";
            const statusTone =
              STATUS_TONES[snapshot.status] ?? STATUS_TONES.not_started;
...
const OnboardingSummaryCard = ({ label, value, tone }) => {
  const palette = {
    success: {
      bg: "bg-emerald-50",
  ...
  return (
    <div
      className={`rounded-3xl border border-[#EAECF0] ${palette.bg} p-4 shadow-sm`}
    >
      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
        {label}
      </p>
