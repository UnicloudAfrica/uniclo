...
const AddSubnet = ({ isOpen, onClose, projectId }) => {
  ...
  const { data: existingSubnets, isFetching: isFetchingSubnets } =
    useFetchClientSubnets(projectId, formData.region, {
      enabled: !!projectId && !!formData.region,
    });
  ...
  const validateForm = () => {
    ...
    const selectedVpc = vpcs?.find(
      (v) => String(v.id) === String(formData.vpc_id)
    );
  ...
  const computeSuggestions = () => {
    ...
    try {
      ...
      const used = (existingSubnets || [])
        .filter(
          (s) =>
            String(s.vpc_id || s.network_id || s.vpc?.id) ===
      ...
      const alignedStart =
        (v.start + ((blockSize - (v.start % blockSize)) % blockSize)) >>> 0;
      let out = [];
      for (
        let cur = alignedStart;
        cur + blockSize - 1 <= v.end;
        cur += blockSize
      ) {
        const curEnd = (cur + blockSize - 1) >>> 0;
        // check overlap with any used range
        const hasOverlap = used.some((r) =>
          ...
          {formData.region &&
            (!edgeConfig ||
              ...
              <div className="w-full mb-4 p-3 rounded-lg bg-yellow-50 border border-yellow-200 text-sm text-yellow-800">
                Edge configuration is missing for this project. Subnet creation
                is disabled until an edge network and IP pool are assigned.
              </div>
            )}
              ...
              <div className="flex gap-2 items-center">
                ...
                <div className="flex items-center gap-1 text-xs">
                  ...
                  <select
                    value={suggestPrefix}
                    onChange={(e) =>
                      setSuggestPrefix(parseInt(e.target.value, 10))
                    }
