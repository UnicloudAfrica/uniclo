...
const InlinePaymentPanel = ({ transactionData, onPaymentComplete, onModifyOrder }) => {
  ...
  useEffect(() => {
    if ((paymentStatus === "pending" || paymentStatus === "transfer_pending") && transaction?.id) {
  ...
  const handlePaymentOptionChange = (optionId: any) => {
  ...
  const handlePayNow = () => {
    ...
    if (selectedPaymentOption.payment_type?.toLowerCase() === "card") {
      if (!selectedPaymentOption.transaction_reference) {
        ToastUtils.error("Payment reference not ready yet. Please try again in a moment.");
        return;
      }
      ...
      if (!window.PaystackPop || typeof window.PaystackPop.setup !== "function") {
        ToastUtils.error("Unable to initialize Paystack. Please refresh the page and try again.");
        return;
      }

      try {
        const amountMinorUnits = Math.round(Number(selectedPaymentOption.total || 0) * 100);
      ...
      ToastUtils.info("Bank transfer details generated. Complete the transfer and refresh status.");
    }
  };
  ...
  const formatCurrency = (value: any, overrideCurrency: any) => {
  ...
  const baseAmount = Number(breakdown.base_amount ?? transaction?.amount ?? order?.total ?? 0);
  const selectedTotal = Number(breakdown.grand_total ?? selectedPaymentOption?.total ?? baseAmount);
  const gatewayFee = Number(breakdown.total_fees ?? Math.max(selectedTotal - baseAmount, 0));
      ...
      >
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex items-start gap-3">
            <div className="rounded-2xl bg-white p-2 shadow-sm">{getStatusIcon()}</div>
            <div className="space-y-1">
              <h3 className="text-lg font-semibold text-slate-900 sm:text-xl">Complete payment</h3>
        ...
        >
          <div className="flex flex-col gap-2 text-sm sm:flex-row sm:items-center sm:justify-between">
            ...
            {timeRemaining && paymentStatus === "pending" && (
              <span className="font-medium" style={{ color: designTokens.colors.warning[600] }}>
              ...
              <div className="space-y-4 text-sm">
                ...
                {paymentGatewayOptions.length > 1 && (
                  <div className="col-span-full space-y-2">
                    ...
                    >
                      {paymentGatewayOptions.map((option: any) => {
                        const optionBreakdown = option.charge_breakdown || {};
                        const optionCurrency = optionBreakdown.currency || breakdownCurrency;
                        const optionTotal = Number(
                          optionBreakdown.grand_total ?? option.total ?? 0
                        );
                        const optionFee = Number(
                          optionBreakdown.total_fees ?? Math.max(optionTotal - baseAmount, 0)
                        );
                        return (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-3 space-y-1 text-xs text-slate-600">
                        ...
                        {osImage?.name && (
                          <div>
                            <span className="font-medium text-slate-700">OS:</span> {osImage.name}
                          </div>
                        )}
                        {(primaryVolume?.name || primaryVolume?.size_gb) && (
                          <div>
                            <span className="font-medium text-slate-700">Primary volume:</span>{" "}
                            {(primaryVolume.name || "Volume").trim()} • {primaryVolume.size_gb} GB
                          </div>
                        )}
                        {additionalVolumes.length > 0 && (
                          <div>
                            <span className="font-medium text-slate-700">Additional volumes:</span>{" "}
                            {additionalVolumes
                              .map((vol: any) => {
                        ...
                        {config.security_groups?.length > 0 && (
                          <div>
                            <span className="font-medium text-slate-700">Security groups:</span>{" "}
                  ...
                  >
                    <div className="flex flex-col">
                      <span className="font-medium text-slate-800">
                        {item.description ||
                          (item.instance?.name ? `${item.instance.name}` : `Line ${item.id}`)}
                      </span>
