...
const ViewSubnetModal = ({ isOpen, onClose, subnet }: any) => {
  ...
  const details = {
    ...
    "VPC ID":
      subnet.vpc_id || subnet.network_id || subnet.vpc?.provider_resource_id || subnet.vpc?.id,
    State: subnet.state,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[1000] font-Outfit">
      <div className="bg-white rounded-[24px] max-w-[540px] w-full mx-4">
        <div className="flex items-center justify-between px-6 py-4 border-b bg-[var(--theme-surface-alt)] rounded-t-[24px]">
          <h2 className="text-lg font-semibold text-[var(--theme-text-color)]">Subnet Details</h2>
          <button
            onClick={onClose}
