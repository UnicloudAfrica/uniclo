...
import { useFetchClientRegions } from "../../../hooks/clientHooks/regionHooks";
...
const includesSiloStorageKeyword = (candidates = []) =>
  candidates.some((value) => SILO_STORAGE_KEYWORDS.some((keyword) => value.includes(keyword)));

const isSiloStoragePricing = (pricing: any) => {
  ...
  if (includesSiloStorageKeyword(typeCandidates)) {
    return true;
  }
  ...
  if (includesSiloStorageKeyword(nameCandidates)) {
    return true;
  }
...
const buildTierLabel = (pricing, currencyOverride, selectedCurrency) => {
  if (!pricing) return "Silo Storage tier";
  const name =
    pricing.product?.name ||
    pricing.product_name ||
    pricing.product?.product_name ||
...
const ObjectStorageCreateForm: React.FC<ObjectStorageCreateFormProps> = ({
  persona = "admin",
  wrapper,
  overviewPath,
  onBack,
  tenantId,
  userId,
  enableFastTrack = persona === "admin",
  showCustomerContext = persona === "admin",
  showPriceOverride = persona === "admin",
}) => {
  const navigate = useNavigate();
  const [searchParams, setSearchParams] = useSearchParams();
  const initialMode =
    enableFastTrack && searchParams.get("mode") === "fast-track" ? "fast-track" : "standard";
  ...
  const isAdminContext = persona === "admin";
  const resolvedOverviewPath =
    overviewPath ||
    (persona === "admin"
      ? "/admin-dashboard/object-storage"
      : persona === "tenant"
        ? "/dashboard/object-storage"
        : "/client-dashboard/object-storage");

  const adminCustomerContext = useCustomerContext({
    enabled: isAdminContext && showCustomerContext,
  });

  const defaultContextType =
    persona === "tenant" ? "tenant" : persona === "client" ? "user" : "unassigned";
  ...
  const contextType = isAdminContext ? adminCustomerContext.contextType : localContextType;
  const setContextType = isAdminContext ? adminCustomerContext.setContextType : setLocalContextType;
  const selectedTenantId = isAdminContext ? adminCustomerContext.selectedTenantId : localTenantId;
  const setSelectedTenantId = isAdminContext
    ? adminCustomerContext.setSelectedTenantId
    : setLocalTenantId;
  const selectedUserId = isAdminContext ? adminCustomerContext.selectedUserId : localUserId;
  const setSelectedUserId = isAdminContext
    ? adminCustomerContext.setSelectedUserId
    : setLocalUserId;
  const tenants = isAdminContext ? adminCustomerContext.tenants : [];
  const isTenantsFetching = isAdminContext ? adminCustomerContext.isTenantsFetching : false;
  const userPool = isAdminContext ? adminCustomerContext.userPool : [];
  const isUsersFetching = isAdminContext ? adminCustomerContext.isUsersFetching : false;
  ...
  const adminRegionsQuery = useFetchRegions({ enabled: persona === "admin" });
  const tenantRegionsQuery = useFetchTenantRegions({ enabled: persona === "tenant" });
  const clientRegionsQuery = useFetchClientRegions({ enabled: persona === "client" });

  const regions =
    persona === "tenant"
      ? tenantRegionsQuery.data || []
      : persona === "client"
        ? clientRegionsQuery.data || []
        : adminRegionsQuery.data || [];
  const isRegionsLoading =
    persona === "tenant"
      ? tenantRegionsQuery.isFetching
      : persona === "client"
        ? clientRegionsQuery.isFetching
        : adminRegionsQuery.isFetching;
  ...
  const tierCatalog = useMemo(() => {
    ...
    rawRows.forEach((row: any) => {
      ...
      const ensureSilo = (siloKey: any) => {
        if (!catalog.has(siloKey)) {
          catalog.set(siloKey, {
            options: [],
            map: new Map(),
          });
        }
        return catalog.get(siloKey);
      };
      ...
      const regionSilo = ensureSilo(regionKey);
      regionSilo.options.push({ value: key, label });
      regionSilo.map.set(key, composite);

      const globalSilo = ensureSilo(GLOBAL_TIER_KEY);
      const globalKey = makeTierKey(GLOBAL_TIER_KEY, composite);
      globalSilo.options.push({ value: globalKey, label });
      globalSilo.map.set(globalKey, composite);
    });
  ...
  const resolvedProfiles = useMemo(() => {
    return serviceProfiles.map((profile: any) => {
      ...
      const regionSilo = regionKey ? tierCatalog.get(regionKey) : null;
      const fallbackSilo = tierCatalog.get(GLOBAL_TIER_KEY);
      const catalogEntry = regionSilo || fallbackSilo;
      const usingFallbackCatalog = Boolean(regionKey && !regionSilo && fallbackSilo);
  ...
  const backendPricingLines = useMemo(() => {
    ...
    return lines.map((line, index) => {
      ...
      return {
        ...
        name: line.name || line.label || "Silo Storage tier",
  ...
  const fallbackPlanLabel = useMemo(() => {
    const tierName = getTierDisplayName(primaryProfile?.tierRow) || "Silo Storage tier";
    if (contextType) {
  ...
  const handleModeChange = (nextMode: any) => {
    ...
    if (nextMode === "fast-track" && !enableFastTrack) return;
    setMode(nextMode);
    setSearchParams((prevParams) => {
      const params = new URLSearchParams(prevParams);
      if (nextMode === "fast-track" && enableFastTrack) {
  ...
  const validateWorkflowStep = () => {
    ...
    if (contextType === "tenant" && !selectedTenantId) {
      workflowErrors.assignment = showCustomerContext
        ? "Select the tenant for this request."
        : "Tenant context is required for this request.";
    }
    if (contextType === "user") {
      if (!selectedTenantId && showCustomerContext) {
        workflowErrors.assignment = "Select the tenant before choosing a user.";
      } else if (!selectedUserId) {
        workflowErrors.assignment = showCustomerContext
          ? "Select the user to charge for this request."
          : "User context is required for this request.";
      }
    }
  ...
  const validateServiceStep = () => {
    ...
    resolvedProfiles.forEach((profile: any) => {
      ...
      if (!profile.tierKey) {
        profileErrs.tierKey = "Select a Silo Storage tier.";
  ...
  const submitOrder = async (event, fastTrackOverride, options = {}) => {
    ...
    try {
      ...
      const lineItems = resolvedProfiles.map((profile: any) => {
        const tierName = profile.name || getTierDisplayName(profile.tierRow) || "Silo Storage tier";
        const rawTierId =
      ...
      const objectStorageItems = lineItems.map((line, index) => {
        ...
        return {
          ...
          name: line.name || line.tierName || `Silo Storage ${line.region}`,
          metadata: {
      ...
      const toastMessage =
        ...
        (isFastTrack
          ...
          : "Silo Storage plan recorded successfully.");
      ...
      if (submissionFastTrack) {
        ...
        if (accountId) {
          ...
          // Fallback: navigate to Silo Storage list
          setTimeout(() => {
      ...
      console.error("Failed to submit Silo Storage order", error);
      const message = error?.message || "Unable to create the Silo Storage record.";
    ...
    try {
      ...
      const authSnapshot =
        persona === "tenant"
          ...
          : persona === "client"
            ? useClientAuthStore.getState()
            : useAdminAuthStore.getState();
      const { user: currentUser, userEmail: currentUserEmail } = authSnapshot || {};
      const payerEmail =
  ...
  const renderModeSwitch = () => (
    <ObjectStoragePlanSelector
      ...
      standardDescription="Process Silo Storage payment and capture billing details."
          ...
          <div className="flex flex-wrap justify-end gap-3">
            ...
            {isLastStep && !isFastTrack && (
              ...
              >
                ...
                {isSubmitting ? "Submitting..." : "Process Silo Payment"}
              </button>
            )}
            {isLastStep && isFastTrack && (
              <button
  ...
  const headerTitle = isFastTrack ? "Fast-track Silo Storage" : "Create Silo Storage plan";
  const headerDescription = isFastTrack
    ? "Provision Silo Storage for tenants that already have executive approval, skipping the invoice wait."
    : "Capture commercial details for a new Silo Storage plan and keep provisioning workflows aligned.";
  const headerActions = (
    <ModernButton variant="outline" onClick={() => navigate(resolvedOverviewPath)}>
      Back to overview
    </ModernButton>
  );
                    ...
                    >
                      ...
                      <div className="mt-4 grid gap-4 md:grid-cols-2">
                        ...
                        <ModernSelect
                          ...
                          onChange={(event) => handleRegionChange(profile.id, event.target.value)}
                          options={[
                            {
                              value: "",
                        ...
                        <ModernInput
                          ...
                          onChange={(event) => handleMonthsChange(profile.id, event.target.value)}
                        ...
                        <ModernInput
                          ...
                          helper={
                            showPriceOverride
                              ? unitPriceHelper
                              : "Pricing is controlled by the catalog for this role."
                          }
                          error={profileError.unitPrice}
                          disabled={!profile.tierKey || !showPriceOverride}
                        />
                      </div>
                      {tierName && (
              ...
              {isPaymentComplete && (
                <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
                  ...
                  <p className="text-xs text-emerald-700">
                    This transaction is confirmed. You can continue to review without making another
                    payment.
                  </p>
                </div>
              )}
                ...
                <div className="space-y-2">
                  {(backendPricingLines || resolvedProfiles).map((entry, index) => {
                    ...
                    const lineName = isBackend
                      ? `${line.region || ""} ${line.name || ""}`.trim() || "Silo Storage tier"
                      : line.region || "N/A";
                    const detail = isBackend
                      ? `${line.name || "Silo Storage tier"}${line.months ? ` • ${line.months} month${line.months === 1 ? "" : "s"}` : ""}`
                      : `${line.tierName || "Silo Storage tier"} • ${line.months} month${line.months === 1 ? "" : "s"}`;
            ...
            <div className="rounded-xl bg-gray-50 p-4 text-xs text-gray-600">
              ...
              {hasCurrencyMismatch && (
                <p className="mt-1 text-[11px] text-amber-600">
                  Multiple currencies detected. Totals are displayed in {summaryTotals.currency} for
                  quick review.
                </p>
              )}
            </div>
...
export default ObjectStorageCreateForm;
